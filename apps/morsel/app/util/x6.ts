...
import { invertColorHelmlab, invertSvgColors } from "@modelscript/core";
...
function invertAllColors(str: string): string {
  return str.replace(COLOR_VALUE_RE, (match) => invertColorHelmlab(match));
}

const COLOR_ATTRS = new Set(["fill", "stroke", "stop-color", "color"]);

/**
 * Recursively walk an X6Markup tree and return a new tree with all
 * colors Helmlab-inverted. The original tree is not mutated.
 */
export function invertMarkupColors(markup: X6Markup, isDark: boolean): X6Markup {
  if (!isDark) return markup;
  const out: X6Markup = { ...markup };
  if (out.attrs) {
    const newAttrs: Record<string, string | number | undefined> = {};
    for (const [key, val] of Object.entries(out.attrs)) {
      if (val == null || typeof val === "number") {
        newAttrs[key] = val;
      } else if (key === "style") {
        newAttrs[key] = invertAllColors(val as string);
  ...
  if (out.children) {
    out.children = out.children.map((child) => invertMarkupColors(child, true));
  }
  return out;
}
...
export function applyCoordinateSystemX6(markup: X6Markup, coordinateSystem?: ICoordinateSystem, isRoot = true): void {
...
export function applyLineArrowsX6(shape: X6Markup, graphicItem: ILine): void {
  ...
  // TODO: Re-enable arrow markers when X6 markup supports them.
