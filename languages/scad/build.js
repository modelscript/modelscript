...
if (isParserUpToDate()) {
  console.log("[scad] WebAssembly parser is up to date, skipping GLR & asc build. (use --force to rebuild)");
  if (!fs.existsSync(distSrcGenDir) && fs.existsSync(path.join(__dirname, "src-gen"))) {
    fs.cpSync(path.join(__dirname, "src-gen"), distSrcGenDir, { recursive: true });
  }
} else {
