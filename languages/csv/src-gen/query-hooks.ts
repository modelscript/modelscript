...
function buildQueryHooks(): Map<string, QueryHooks> {
  ...
  // SourceFile: instantiate, resolveSimpleName
  {
    const $ = new Proxy(
      {},
      {
    ...
    if (ruleAst && (ruleAst as Record<string, unknown>).type === "def") {
      ...
      if (Object.keys(merged).length > 0) {
        hooks.set("SourceFile", merged);
      }
    }
  }

  // CSVVirtualComponent: resolvedType, classInstance, variability, causality, isOuter, isInner, isProtected, isFinal, resolvedArrayDimensions, arrayDimensions, effectiveModification
  {
    const $ = new Proxy(
      {},
      {
    ...
    if (ruleAst && (ruleAst as Record<string, unknown>).type === "def") {
      ...
      if (Object.keys(merged).length > 0) {
        hooks.set("CSVVirtualComponent", merged);
      }
    }
  }
