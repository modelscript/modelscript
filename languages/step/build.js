...
if (isParserUpToDate()) {
  console.log("[step] WebAssembly parser is up to date, skipping GLR & asc build. (use --force to rebuild)");
} else {
...
const languagePath = path.join(__dirname, "src", "language.ts");
const result = buildParser(stepLanguage, { sourcePath: languagePath });
...
console.log("[step] Compiling WebAssembly parser with asc...");
const [ascBin, ...ascPrefixArgs] = ascPath.startsWith("npx") ? ["npx", "asc"] : [ascPath];
execFileSync(ascBin, [...ascPrefixArgs, parserTs, "-o", outWasm, "--exportRuntime", "--enable", "threads", "--optimize", "--runtime", "stub", "--disableWarning", "235"], {
  stdio: "inherit",
  cwd: __dirname,
});
console.log("[step] WebAssembly parser built successfully -> " + outWasm);
