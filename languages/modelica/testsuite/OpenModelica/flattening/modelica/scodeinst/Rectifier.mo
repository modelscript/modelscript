...
// Result:
// function PowerSystems.Utilities.Transforms.der2_park "2nd derivative of Park transform"
//   input Real theta(quantity = "Angle", unit = "rad", displayUnit = "deg") "transformation angle";
...
//   output Real[3, 3] der2_P "d2/dt2 park";
//   protected constant Real s23 = 0.816496580927726;
//   protected constant Real dph_b = 2.0943951023931953;
//   protected constant Real dph_c = 4.1887902047863905;
//   protected Real[3] c;
//   protected Real[3] s;
//   protected Real s23omega_dot;
//   protected Real s23omega2;
// algorithm
//   s23omega_dot := s23 * omega_dot;
//   s23omega2 := s23 * omega * omega;
//   c := array(cos({theta, theta - dph_b, theta - dph_c}[$i4]) for $i4 in 1:3);
//   s := array(sin({theta, theta - dph_b, theta - dph_c}[$i5]) for $i5 in 1:3);
//   der2_P := {{-(s23omega_dot * s[1] + s23omega2 * c[1]), -(s23omega_dot * s[2] + s23omega2 * c[2]), -(s23omega_dot * s[3] + s23omega2 * c[3])}, {s23omega2 * s[1] - s23omega_dot * c[1], s23omega2 * s[2] - s23omega_dot * c[2], s23omega2 * s[3] - s23omega_dot * c[3]}, {0.0, 0.0, 0.0}};
// end PowerSystems.Utilities.Transforms.der2_park;
//
// function PowerSystems.Utilities.Transforms.der_park "Derivative of Park transform"
//   input Real theta(quantity = "Angle", unit = "rad", displayUnit = "deg") "transformation angle";
//   input Real omega(quantity = "AngularFrequency", unit = "rad/s") "d/dt theta";
//   output Real[3, 3] der_P "d/dt park";
//   protected constant Real s23 = 0.816496580927726;
...
//   protected Real[3] c;
//   protected Real[3] s;
//   protected Real s23omega;
// algorithm
//   s23omega := s23 * omega;
//   c := array(cos({theta, theta - dph_b, theta - dph_c}[$i2]) for $i2 in 1:3);
//   s := array(sin({theta, theta - dph_b, theta - dph_c}[$i3]) for $i3 in 1:3);
//   der_P := {{-s23omega * s[1], -s23omega * s[2], -s23omega * s[3]}, {-s23omega * c[1], -s23omega * c[2], -s23omega * c[3]}, {0.0, 0.0, 0.0}};
// end PowerSystems.Utilities.Transforms.der_park;
//
// function PowerSystems.Utilities.Transforms.park "Park transform"
//   input Real theta(quantity = "Angle", unit = "rad", displayUnit = "deg") "transformation angle";
//   output Real[3, 3] P "Park transformation matrix";
//   protected constant Real s13 = 0.5773502691896257;
//   protected constant Real s23 = 0.816496580927726;
//   protected constant Real dph_b = 2.0943951023931953;
//   protected constant Real dph_c = 4.1887902047863905;
//   protected Real[3] c;
//   protected Real[3] s;
// algorithm
//   c := array(cos({theta, theta - dph_b, theta - dph_c}[$i0]) for $i0 in 1:3);
//   s := array(sin({theta, theta - dph_b, theta - dph_c}[$i1]) for $i1 in 1:3);
//   P := {{s23 * c[1], s23 * c[2], s23 * c[3]}, {-s23 * s[1], -s23 * s[2], -s23 * s[3]}, {s13, s13, s13}};
// end PowerSystems.Utilities.Transforms.park;
//
// function Rectifier_total.ind.PS.map "Map vector of higher order phase system to this phase system"
//   input Real[:] x;
//   output Real[3] y;
// algorithm
//   y := x[1:3];
// end Rectifier_total.ind.PS.map;
//
// function Rectifier_total.ind.j
//   input Real[:] x;
//   output Real[size(x, 1)] y;
// algorithm
//   y := cat(1, {-x[2], x[1]}, fill(0.0, size(x, 1) - 2));
// end Rectifier_total.ind.j;
//
// function Rectifier_total.meterAC.PS.j "Rotation(pi/2) of vector around {0,0,1} and projection on orth plane"
//   input Real[:] x;
//   output Real[size(x, 1)] y;
// algorithm
//   y := cat(1, {-x[2], x[1]}, fill(0.0, size(x, 1) - 2));
// end Rectifier_total.meterAC.PS.j;
//
// function Rectifier_total.meterAC.PS.phasePowers_vi "Return phase powers"
//   input Real[3] v(quantity = {"Voltage.ThreePhase_dq0", "Voltage.ThreePhase_dq0", "Voltage.ThreePhase_dq0"}, unit = {"V", "V", "V"}, displayUnit = {"kV", "kV", "kV"}, nominal = {1000.0, 1000.0, 1000.0}) "phase voltages";
//   input Real[3] i(quantity = {"Current.ThreePhase_dq0", "Current.ThreePhase_dq0", "Current.ThreePhase_dq0"}, unit = {"A", "A", "A"}, nominal = {1.0, 1.0, 1.0}) "phase currents";
//   output Real[3] p(quantity = {"Power", "Power", "Power"}, unit = {"W", "W", "W"}, displayUnit = {"MW", "MW", "MW"}) "phase powers";
// algorithm
//   p := {v[1] * i[1] + v[2] * i[2], -Rectifier_total.meterAC.PS.j(v[1:2]) * i[1:2], v[3] * i[3]};
// end Rectifier_total.meterAC.PS.phasePowers_vi;
//
// function Rectifier_total.meterAC.park
//   input Real theta(quantity = "Angle", unit = "rad", displayUnit = "deg") "transformation angle";
//   output Real[3, 3] P "Park transformation matrix";
//   protected constant Real s13 = 0.5773502691896257;
//   protected constant Real s23 = 0.816496580927726;
...
//   protected Real[3] s;
// algorithm
//   c := array(cos({theta, theta - dph_b, theta - dph_c}[$i6]) for $i6 in 1:3);
//   s := array(sin({theta, theta - dph_b, theta - dph_c}[$i7]) for $i7 in 1:3);
//   P := {{s23 * c[1], s23 * c[2], s23 * c[3]}, {-s23 * s[1], -s23 * s[2], -s23 * s[3]}, {s13, s13, s13}};
// end Rectifier_total.meterAC.park;
//
// function Rectifier_total.meterAC.rot_dq
...
//   s := sin(theta);
//   R_dq := {{c, -s}, {s, c}};
// end Rectifier_total.meterAC.rot_dq;
//
// function Rectifier_total.meterAC.v2vpp_abc
//   input Real[3] v_abc(quantity = {"ElectricPotential", "ElectricPotential", "ElectricPotential"}, unit = {"V/V", "V/V", "V/V"});
//   output Real[3] vpp_abc(quantity = {"ElectricPotential", "ElectricPotential", "ElectricPotential"}, unit = {"V/V", "V/V", "V/V"});
// algorithm
//   vpp_abc := {v_abc[2] - v_abc[3], v_abc[3] - v_abc[1], v_abc[1] - v_abc[2]};
// end Rectifier_total.meterAC.v2vpp_abc;
//
// function Rectifier_total.rectifier.rectifier.PS.map "Map vector of higher order phase system to this phase system"
//   input Real[:] x;
//   output Real[3] y;
// algorithm
//   y := x[1:3];
// end Rectifier_total.rectifier.rectifier.PS.map;
//
// function Rectifier_total.vAC.PS.map "Map vector of higher order phase system to this phase system"
//   input Real[:] x;
//   output Real[3] y;
// algorithm
//   y := x[1:3];
// end Rectifier_total.vAC.PS.map;
//
// class Rectifier_total "Rectifier"
//   final parameter enumeration(Parameter, Signal, Average) system.fType = PowerSystems.Types.SystemFrequency.Parameter "system frequency type";
//   final parameter Real system.f(quantity = "Frequency", unit = "Hz") = 50.0 "frequency if type is parameter, else initial frequency";
...
//   final parameter enumeration(Synchron, Inertial) system.refType = PowerSystems.Types.ReferenceFrame.Inertial "reference frame (3-phase)";
//   final parameter enumeration(FreeInitial, FixedInitial, SteadyInitial, SteadyState) system.dynType = PowerSystems.Types.Dynamics.FixedInitial "transient or steady-state model";
...
//   final parameter Boolean system.synRef = false;
//   discrete Real system.initime(quantity = "Time", unit = "s");
//   Real system.theta(quantity = "Angle", unit = "rad", displayUnit = "deg", start = 0.0, stateSelect = StateSelect.default) "system angle";
//   Real system.thetaRel(quantity = "Angle", unit = "rad", displayUnit = "deg") = system.theta - system.thetaRef "angle relative to reference frame";
//   Real system.thetaRef(quantity = "Angle", unit = "rad", displayUnit = "deg") = 0.0 "angle of reference frame";
...
//   protected Real system.omega_internal;
//   Real transPh.y[1] "output signal-vector";
//   Real transPh.y[2] "output signal-vector";
//   final parameter Integer transPh.n = 2 "dim of output signal-vector";
//   parameter Real transPh.t_change(quantity = "Time", unit = "s") = 0.1 "time when change";
//   parameter Real transPh.t_duration(quantity = "Time", unit = "s") = 0.1 "transition duration";
//   parameter Real transPh.a_start = 2.0 "initial norm |y|";
//   parameter Real transPh.a_end = 1.0 "final norm |y|";
//   parameter Real transPh.ph_start(quantity = "Angle", unit = "rad", displayUnit = "deg") = 0.0 "initial phase (y)";
//   parameter Real transPh.ph_end(quantity = "Angle", unit = "rad", displayUnit = "deg") = 0.0 "final phase (y)";
//   protected final parameter Real transPh.coef(quantity = "Frequency", unit = "Hz") = 5.43656365691809 / transPh.t_duration;
//   Real vAC.term.v[1](quantity = "Voltage.ThreePhase_dq0", unit = "V", displayUnit = "kV", nominal = 1000.0) "voltage vector";
//   Real vAC.term.v[2](quantity = "Voltage.ThreePhase_dq0", unit = "V", displayUnit = "kV", nominal = 1000.0) "voltage vector";
//   Real vAC.term.v[3](quantity = "Voltage.ThreePhase_dq0", unit = "V", displayUnit = "kV", nominal = 1000.0) "voltage vector";
//   Real vAC.term.i[1](quantity = "Current.ThreePhase_dq0", unit = "A", nominal = 1.0) "current vector";
//   Real vAC.term.i[2](quantity = "Current.ThreePhase_dq0", unit = "A", nominal = 1.0) "current vector";
//   Real vAC.term.i[3](quantity = "Current.ThreePhase_dq0", unit = "A", nominal = 1.0) "current vector";
//   Real vAC.term.theta[1] "optional vector of phase angles";
//   Real vAC.term.theta[2] "optional vector of phase angles";
//   final parameter Boolean vAC.puUnits = true "= true, if scaled with nom. values (pu), else scaled with 1 (SI)";
//   final parameter Real vAC.V_nom(quantity = "ElectricPotential", unit = "V", displayUnit = "kV", min = 0.0) = 100.0 "nominal Voltage (= base for pu)";
//   final parameter Real vAC.S_nom(quantity = "Power", unit = "VA", displayUnit = "MVA", min = 0.0) = 1.0 "nominal Power (= base for pu)";
//   Real vAC.neutral.v(quantity = "ElectricPotential", unit = "V") "Potential at the pin";
//   Real vAC.neutral.i(quantity = "ElectricCurrent", unit = "A") "Current flowing into the pin";
//   protected final parameter Real vAC.V_base(quantity = "Voltage.ThreePhase_dq0", unit = "V", displayUnit = "kV", nominal = 1000.0) = 100.0;
//   protected Real vAC.theta(quantity = "Angle", unit = "rad", displayUnit = "deg", stateSelect = StateSelect.prefer) "absolute angle";
//   final parameter enumeration(Parameter, Signal, System) vAC.fType = PowerSystems.Types.SourceFrequency.System "frequency type";
//   final parameter Real vAC.f(quantity = "Frequency", unit = "Hz") = 50.0 "frequency if type is parameter";
//   final parameter Boolean vAC.use_vPhasor_in = true "= true to use input signal vPhasor_in, otherwise use fixed values";
//   Real vAC.vPhasor_in[1] "{abs(voltage), phase}";
//   Real vAC.vPhasor_in[2] "{abs(voltage), phase}";
//   protected Real vAC.omega_internal "Needed to connect to conditional connector";
//   protected Real vAC.vPhasor_internal[1] "Needed to connect to conditional connector";
//   protected Real vAC.vPhasor_internal[2] "Needed to connect to conditional connector";
//   parameter Real vAC.v0(quantity = "ElectricPotential", unit = "V/V") = 1.0 "fixed voltage";
//   parameter Real vAC.alpha0(quantity = "Angle", unit = "rad", displayUnit = "deg") = 0.0 "fixed phase angle";
//   protected Real vAC.V(quantity = "Voltage.ThreePhase_dq0", unit = "V", displayUnit = "kV", nominal = 1000.0);
//   protected Real vAC.alpha(quantity = "Angle", unit = "rad", displayUnit = "deg");
//   protected Real vAC.phi(quantity = "Angle", unit = "rad", displayUnit = "deg");
//   Real ind.term_p.v[1](quantity = "Voltage.ThreePhase_dq0", unit = "V", displayUnit = "kV", nominal = 1000.0) "voltage vector";
//   Real ind.term_p.v[2](quantity = "Voltage.ThreePhase_dq0", unit = "V", displayUnit = "kV", nominal = 1000.0) "voltage vector";
//   Real ind.term_p.v[3](quantity = "Voltage.ThreePhase_dq0", unit = "V", displayUnit = "kV", nominal = 1000.0) "voltage vector";
//   Real ind.term_p.i[1](quantity = "Current.ThreePhase_dq0", unit = "A", nominal = 1.0) "current vector";
//   Real ind.term_p.i[2](quantity = "Current.ThreePhase_dq0", unit = "A", nominal = 1.0) "current vector";
//   Real ind.term_p.i[3](quantity = "Current.ThreePhase_dq0", unit = "A", nominal = 1.0) "current vector";
//   Real ind.term_p.theta[1] "optional vector of phase angles";
//   Real ind.term_p.theta[2] "optional vector of phase angles";
//   Real ind.term_n.v[1](quantity = "Voltage.ThreePhase_dq0", unit = "V", displayUnit = "kV", nominal = 1000.0) "voltage vector";
//   Real ind.term_n.v[2](quantity = "Voltage.ThreePhase_dq0", unit = "V", displayUnit = "kV", nominal = 1000.0) "voltage vector";
//   Real ind.term_n.v[3](quantity = "Voltage.ThreePhase_dq0", unit = "V", displayUnit = "kV", nominal = 1000.0) "voltage vector";
//   Real ind.term_n.i[1](quantity = "Current.ThreePhase_dq0", unit = "A", nominal = 1.0) "current vector";
//   Real ind.term_n.i[2](quantity = "Current.ThreePhase_dq0", unit = "A", nominal = 1.0) "current vector";
//   Real ind.term_n.i[3](quantity = "Current.ThreePhase_dq0", unit = "A", nominal = 1.0) "current vector";
//   Real ind.term_n.theta[1] "optional vector of phase angles";
//   Real ind.term_n.theta[2] "optional vector of phase angles";
//   final parameter Boolean ind.puUnits = true "= true, if scaled with nom. values (pu), else scaled with 1 (SI)";
//   final parameter Real ind.V_nom(quantity = "ElectricPotential", unit = "V", displayUnit = "kV", min = 0.0) = 100.0 "nominal Voltage (= base for pu)";
//   final parameter Real ind.S_nom(quantity = "Power", unit = "VA", displayUnit = "MVA", min = 0.0) = 1000.0 "nominal Power (= base for pu)";
//   final parameter Real ind.f_nom(quantity = "Frequency", unit = "Hz") = 50.0 "nominal frequency";
//   final parameter enumeration(FreeInitial, FixedInitial, SteadyInitial, SteadyState) ind.dynType = PowerSystems.Types.Dynamics.FixedInitial "transient or steady-state model";
//   parameter Real ind.v_start[1](quantity = "Voltage.ThreePhase_dq0", unit = "V", displayUnit = "kV", nominal = 1000.0) = 0.0 "start value of voltage drop";
//   parameter Real ind.v_start[2](quantity = "Voltage.ThreePhase_dq0", unit = "V", displayUnit = "kV", nominal = 1000.0) = 0.0 "start value of voltage drop";
//   parameter Real ind.v_start[3](quantity = "Voltage.ThreePhase_dq0", unit = "V", displayUnit = "kV", nominal = 1000.0) = 0.0 "start value of voltage drop";
//   parameter Real ind.i_start[1](quantity = "Current.ThreePhase_dq0", unit = "A", nominal = 1.0) = 0.0 "start value of current";
//   parameter Real ind.i_start[2](quantity = "Current.ThreePhase_dq0", unit = "A", nominal = 1.0) = 0.0 "start value of current";
//   parameter Real ind.i_start[3](quantity = "Current.ThreePhase_dq0", unit = "A", nominal = 1.0) = 0.0 "start value of current";
//   Real ind.v[1](quantity = "Voltage.ThreePhase_dq0", unit = "V", displayUnit = "kV", start = ind.v_start[1], nominal = 1000.0);
//   Real ind.v[2](quantity = "Voltage.ThreePhase_dq0", unit = "V", displayUnit = "kV", start = ind.v_start[2], nominal = 1000.0);
//   Real ind.v[3](quantity = "Voltage.ThreePhase_dq0", unit = "V", displayUnit = "kV", start = ind.v_start[3], nominal = 1000.0);
//   Real ind.i[1](quantity = "Current.ThreePhase_dq0", unit = "A", start = ind.i_start[1], nominal = 1.0);
//   Real ind.i[2](quantity = "Current.ThreePhase_dq0", unit = "A", start = ind.i_start[2], nominal = 1.0);
//   Real ind.i[3](quantity = "Current.ThreePhase_dq0", unit = "A", start = ind.i_start[3], nominal = 1.0);
//   protected Real ind.omega[1](quantity = "AngularFrequency", unit = "rad/s");
//   protected Real ind.omega[2](quantity = "AngularFrequency", unit = "rad/s");
//   parameter Real ind.r(quantity = "Resistance", unit = "Ohm/(V.V/VA)", min = 0.0) = 0.05 "resistance";
//   parameter Real ind.x_s(quantity = "Reactance", unit = "Ohm/Ohm") = 1.0 "self reactance";
//   parameter Real ind.x_m(quantity = "Reactance", unit = "Ohm/Ohm") = 0.0 "mutual reactance, -x_s/2 < x_m < x_s";
//   protected final parameter Real ind.RL_base[1] = 10.0;
//   protected final parameter Real ind.RL_base[2] = 0.03183098861837907;
//   protected final parameter Real ind.R(quantity = "Resistance", unit = "Ohm") = ind.r * ind.RL_base[1];
//   protected final parameter Real ind.L(quantity = "Inductance", unit = "H") = (ind.x_s - ind.x_m) * ind.RL_base[2];
//   protected final parameter Real ind.L0(quantity = "Inductance", unit = "H") = (ind.x_s + 2.0 * ind.x_m) * ind.RL_base[2];
//   Real meterAC.term_p.v[1](quantity = "Voltage.ThreePhase_dq0", unit = "V", displayUnit = "kV", nominal = 1000.0) "voltage vector";
//   Real meterAC.term_p.v[2](quantity = "Voltage.ThreePhase_dq0", unit = "V", displayUnit = "kV", nominal = 1000.0) "voltage vector";
//   Real meterAC.term_p.v[3](quantity = "Voltage.ThreePhase_dq0", unit = "V", displayUnit = "kV", nominal = 1000.0) "voltage vector";
//   Real meterAC.term_p.i[1](quantity = "Current.ThreePhase_dq0", unit = "A", nominal = 1.0) "current vector";
//   Real meterAC.term_p.i[2](quantity = "Current.ThreePhase_dq0", unit = "A", nominal = 1.0) "current vector";
//   Real meterAC.term_p.i[3](quantity = "Current.ThreePhase_dq0", unit = "A", nominal = 1.0) "current vector";
//   Real meterAC.term_p.theta[1] "optional vector of phase angles";
//   Real meterAC.term_p.theta[2] "optional vector of phase angles";
//   Real meterAC.term_n.v[1](quantity = "Voltage.ThreePhase_dq0", unit = "V", displayUnit = "kV", nominal = 1000.0) "voltage vector";
//   Real meterAC.term_n.v[2](quantity = "Voltage.ThreePhase_dq0", unit = "V", displayUnit = "kV", nominal = 1000.0) "voltage vector";
//   Real meterAC.term_n.v[3](quantity = "Voltage.ThreePhase_dq0", unit = "V", displayUnit = "kV", nominal = 1000.0) "voltage vector";
//   Real meterAC.term_n.i[1](quantity = "Current.ThreePhase_dq0", unit = "A", nominal = 1.0) "current vector";
//   Real meterAC.term_n.i[2](quantity = "Current.ThreePhase_dq0", unit = "A", nominal = 1.0) "current vector";
//   Real meterAC.term_n.i[3](quantity = "Current.ThreePhase_dq0", unit = "A", nominal = 1.0) "current vector";
//   Real meterAC.term_n.theta[1] "optional vector of phase angles";
//   Real meterAC.term_n.theta[2] "optional vector of phase angles";
//   final parameter Integer meterAC.signalTrsf = 0 "signal in which reference frame?";
//   final parameter Boolean meterAC.puUnits = true "= true, if scaled with nom. values (pu), else scaled with 1 (SI)";
//   final parameter Real meterAC.V_nom(quantity = "ElectricPotential", unit = "V", displayUnit = "kV", min = 0.0) = 100.0 "nominal Voltage (= base for pu)";
//   final parameter Real meterAC.S_nom(quantity = "Power", unit = "VA", displayUnit = "MVA", min = 0.0) = 1000.0 "nominal Power (= base for pu)";
//   final parameter Boolean meterAC.abc = true "abc inertial";
//   protected Real meterAC.Park[1,1];
//   protected Real meterAC.Park[1,2];
//   protected Real meterAC.Park[1,3];
//   protected Real meterAC.Park[2,1];
//   protected Real meterAC.Park[2,2];
//   protected Real meterAC.Park[2,3];
//   protected Real meterAC.Park[3,1];
//   protected Real meterAC.Park[3,2];
//   protected Real meterAC.Park[3,3];
//   protected Real meterAC.Rot_dq[1,1];
//   protected Real meterAC.Rot_dq[1,2];
//   protected Real meterAC.Rot_dq[2,1];
//   protected Real meterAC.Rot_dq[2,2];
//   final parameter Boolean meterAC.av = true "time average power";
//   final parameter Real meterAC.tcst(quantity = "Time", unit = "s", min = 1e-9) = 0.1 "average time-constant";
//   Real meterAC.p[1](quantity = "Power", unit = "W/W", stateSelect = StateSelect.never);
//   Real meterAC.p[2](quantity = "Power", unit = "W/W", stateSelect = StateSelect.never);
//   Real meterAC.p[3](quantity = "Power", unit = "W/W", stateSelect = StateSelect.never);
//   Real meterAC.p_av[1](quantity = "Power", unit = "W/W");
//   Real meterAC.p_av[2](quantity = "Power", unit = "W/W");
//   Real meterAC.p_av[3](quantity = "Power", unit = "W/W");
//   Real meterAC.v[1](quantity = "ElectricPotential", unit = "V/V", stateSelect = StateSelect.never);
//   Real meterAC.v[2](quantity = "ElectricPotential", unit = "V/V", stateSelect = StateSelect.never);
//   Real meterAC.v[3](quantity = "ElectricPotential", unit = "V/V", stateSelect = StateSelect.never);
//   Real meterAC.vpp[1](quantity = "ElectricPotential", unit = "V/V", stateSelect = StateSelect.never);
//   Real meterAC.vpp[2](quantity = "ElectricPotential", unit = "V/V", stateSelect = StateSelect.never);
//   Real meterAC.i[1](quantity = "ElectricCurrent", unit = "A/A", stateSelect = StateSelect.never);
//   Real meterAC.i[2](quantity = "ElectricCurrent", unit = "A/A", stateSelect = StateSelect.never);
//   Real meterAC.i[3](quantity = "ElectricCurrent", unit = "A/A", stateSelect = StateSelect.never);
//   Real meterAC.v_abc[1](quantity = "ElectricPotential", unit = "V/V", stateSelect = StateSelect.never);
//   Real meterAC.v_abc[2](quantity = "ElectricPotential", unit = "V/V", stateSelect = StateSelect.never);
//   Real meterAC.v_abc[3](quantity = "ElectricPotential", unit = "V/V", stateSelect = StateSelect.never);
//   Real meterAC.vpp_abc[1](quantity = "ElectricPotential", unit = "V/V", stateSelect = StateSelect.never);
//   Real meterAC.vpp_abc[2](quantity = "ElectricPotential", unit = "V/V", stateSelect = StateSelect.never);
//   Real meterAC.vpp_abc[3](quantity = "ElectricPotential", unit = "V/V", stateSelect = StateSelect.never);
//   Real meterAC.i_abc[1](quantity = "ElectricCurrent", unit = "A/A", stateSelect = StateSelect.never);
//   Real meterAC.i_abc[2](quantity = "ElectricCurrent", unit = "A/A", stateSelect = StateSelect.never);
//   Real meterAC.i_abc[3](quantity = "ElectricCurrent", unit = "A/A", stateSelect = StateSelect.never);
//   Real meterAC.v_norm(quantity = "ElectricPotential", unit = "V/V", stateSelect = StateSelect.never);
//   Real meterAC.alpha_v(quantity = "Angle", unit = "rad", displayUnit = "deg", stateSelect = StateSelect.never);
//   Real meterAC.i_norm(quantity = "ElectricCurrent", unit = "A/A", stateSelect = StateSelect.never);
//   Real meterAC.alpha_i(quantity = "Angle", unit = "rad", displayUnit = "deg", stateSelect = StateSelect.never);
//   Real meterAC.cos_phi(stateSelect = StateSelect.never);
//   protected final parameter Real meterAC.V_base(quantity = "Voltage.ThreePhase_dq0", unit = "V", displayUnit = "kV", nominal = 1000.0) = 100.0;
//   protected final parameter Real meterAC.I_base(quantity = "Current.ThreePhase_dq0", unit = "A", nominal = 1.0) = 10.0;
//   protected Real meterAC.pav[1](quantity = "Power", unit = "W/W");
//   protected Real meterAC.pav[2](quantity = "Power", unit = "W/W");
//   protected Real meterAC.pav[3](quantity = "Power", unit = "W/W");
//   Real rectifier.AC.v[1](quantity = "Voltage.ThreePhase_dq0", unit = "V", displayUnit = "kV", nominal = 1000.0) "voltage vector";
//   Real rectifier.AC.v[2](quantity = "Voltage.ThreePhase_dq0", unit = "V", displayUnit = "kV", nominal = 1000.0) "voltage vector";
//   Real rectifier.AC.v[3](quantity = "Voltage.ThreePhase_dq0", unit = "V", displayUnit = "kV", nominal = 1000.0) "voltage vector";
//   Real rectifier.AC.i[1](quantity = "Current.ThreePhase_dq0", unit = "A", nominal = 1.0) "current vector";
//   Real rectifier.AC.i[2](quantity = "Current.ThreePhase_dq0", unit = "A", nominal = 1.0) "current vector";
//   Real rectifier.AC.i[3](quantity = "Current.ThreePhase_dq0", unit = "A", nominal = 1.0) "current vector";
//   Real rectifier.AC.theta[1] "optional vector of phase angles";
//   Real rectifier.AC.theta[2] "optional vector of phase angles";
//   Real rectifier.DC.v[1](quantity = "Voltage.TwoConductor", unit = "V", displayUnit = "kV", nominal = 1000.0) "voltage vector";
//   Real rectifier.DC.v[2](quantity = "Voltage.TwoConductor", unit = "V", displayUnit = "kV", nominal = 1000.0) "voltage vector";
//   Real rectifier.DC.i[1](quantity = "Current.TwoConductor", unit = "A", nominal = 1.0) "current vector";
//   Real rectifier.DC.i[2](quantity = "Current.TwoConductor", unit = "A", nominal = 1.0) "current vector";
//   final parameter Integer rectifier.heat.m(min = 1) = 3 "number of single heat-ports";
//   Real rectifier.heat.ports[1].T(quantity = "ThermodynamicTemperature", unit = "K", displayUnit = "degC", min = 0.0, start = 288.15, nominal = 300.0) "Port temperature";
//   Real rectifier.heat.ports[1].Q_flow(quantity = "Power", unit = "W") "Heat flow rate (positive if flowing from outside into the component)";
//   Real rectifier.heat.ports[2].T(quantity = "ThermodynamicTemperature", unit = "K", displayUnit = "degC", min = 0.0, start = 288.15, nominal = 300.0) "Port temperature";
//   Real rectifier.heat.ports[2].Q_flow(quantity = "Power", unit = "W") "Heat flow rate (positive if flowing from outside into the component)";
//   Real rectifier.heat.ports[3].T(quantity = "ThermodynamicTemperature", unit = "K", displayUnit = "degC", min = 0.0, start = 288.15, nominal = 300.0) "Port temperature";
//   Real rectifier.heat.ports[3].Q_flow(quantity = "Power", unit = "W") "Heat flow rate (positive if flowing from outside into the component)";
//   Real rectifier.rectifier.AC.v[1](quantity = "Voltage.ThreePhase_dq0", unit = "V", displayUnit = "kV", nominal = 1000.0) "voltage vector";
//   Real rectifier.rectifier.AC.v[2](quantity = "Voltage.ThreePhase_dq0", unit = "V", displayUnit = "kV", nominal = 1000.0) "voltage vector";
//   Real rectifier.rectifier.AC.v[3](quantity = "Voltage.ThreePhase_dq0", unit = "V", displayUnit = "kV", nominal = 1000.0) "voltage vector";
//   Real rectifier.rectifier.AC.i[1](quantity = "Current.ThreePhase_dq0", unit = "A", nominal = 1.0) "current vector";
//   Real rectifier.rectifier.AC.i[2](quantity = "Current.ThreePhase_dq0", unit = "A", nominal = 1.0) "current vector";
//   Real rectifier.rectifier.AC.i[3](quantity = "Current.ThreePhase_dq0", unit = "A", nominal = 1.0) "current vector";
//   Real rectifier.rectifier.AC.theta[1] "optional vector of phase angles";
//   Real rectifier.rectifier.AC.theta[2] "optional vector of phase angles";
//   Real rectifier.rectifier.DC.v[1](quantity = "Voltage.TwoConductor", unit = "V", displayUnit = "kV", nominal = 1000.0) "voltage vector";
//   Real rectifier.rectifier.DC.v[2](quantity = "Voltage.TwoConductor", unit = "V", displayUnit = "kV", nominal = 1000.0) "voltage vector";
//   Real rectifier.rectifier.DC.i[1](quantity = "Current.TwoConductor", unit = "A", nominal = 1.0) "current vector";
//   Real rectifier.rectifier.DC.i[2](quantity = "Current.TwoConductor", unit = "A", nominal = 1.0) "current vector";
//   final parameter Integer rectifier.rectifier.heat.m(min = 1) = 3 "number of single heat-ports";
//   Real rectifier.rectifier.heat.ports[1].T(quantity = "ThermodynamicTemperature", unit = "K", displayUnit = "degC", min = 0.0, start = 288.15, nominal = 300.0) "Port temperature";
//   Real rectifier.rectifier.heat.ports[1].Q_flow(quantity = "Power", unit = "W") "Heat flow rate (positive if flowing from outside into the component)";
//   Real rectifier.rectifier.heat.ports[2].T(quantity = "ThermodynamicTemperature", unit = "K", displayUnit = "degC", min = 0.0, start = 288.15, nominal = 300.0) "Port temperature";
//   Real rectifier.rectifier.heat.ports[2].Q_flow(quantity = "Power", unit = "W") "Heat flow rate (positive if flowing from outside into the component)";
//   Real rectifier.rectifier.heat.ports[3].T(quantity = "ThermodynamicTemperature", unit = "K", displayUnit = "degC", min = 0.0, start = 288.15, nominal = 300.0) "Port temperature";
//   Real rectifier.rectifier.heat.ports[3].Q_flow(quantity = "Power", unit = "W") "Heat flow rate (positive if flowing from outside into the component)";
//   protected Real rectifier.rectifier.vDC1(quantity = "Voltage.ThreePhase_dq0", unit = "V", displayUnit = "kV", nominal = 1000.0) = 0.5 * (rectifier.rectifier.DC.v[1] - rectifier.rectifier.DC.v[2]);
//   protected Real rectifier.rectifier.vDC0(quantity = "Voltage.ThreePhase_dq0", unit = "V", displayUnit = "kV", nominal = 1000.0) = 0.5 * (rectifier.rectifier.DC.v[1] + rectifier.rectifier.DC.v[2]);
//   protected Real rectifier.rectifier.iDC1(quantity = "Current.ThreePhase_dq0", unit = "A", nominal = 1.0) = rectifier.rectifier.DC.i[1] - rectifier.rectifier.DC.i[2];
//   protected Real rectifier.rectifier.iDC0(quantity = "Current.ThreePhase_dq0", unit = "A", nominal = 1.0) = rectifier.rectifier.DC.i[1] + rectifier.rectifier.DC.i[2];
//   protected Real rectifier.rectifier.v_dq0[1] "switching function voltage in dq0 representation";
//   protected Real rectifier.rectifier.v_dq0[2] "switching function voltage in dq0 representation";
//   protected Real rectifier.rectifier.v_dq0[3] "switching function voltage in dq0 representation";
//   protected Real rectifier.rectifier.switch_dq0[1] "switching function in dq0 representation";
//   protected Real rectifier.rectifier.switch_dq0[2] "switching function in dq0 representation";
//   protected Real rectifier.rectifier.switch_dq0[3] "switching function in dq0 representation";
//   protected Real rectifier.rectifier.T[1](quantity = "ThermodynamicTemperature", unit = "K", displayUnit = "degC", min = 0.0, start = 288.15, nominal = 300.0) "component temperature";
//   protected Real rectifier.rectifier.T[2](quantity = "ThermodynamicTemperature", unit = "K", displayUnit = "degC", min = 0.0, start = 288.15, nominal = 300.0) "component temperature";
//   protected Real rectifier.rectifier.T[3](quantity = "ThermodynamicTemperature", unit = "K", displayUnit = "degC", min = 0.0, start = 288.15, nominal = 300.0) "component temperature";
//   protected Real rectifier.rectifier.Q_flow[1](quantity = "Power", unit = "W") "component loss-heat flow";
//   protected Real rectifier.rectifier.Q_flow[2](quantity = "Power", unit = "W") "component loss-heat flow";
//   protected Real rectifier.rectifier.Q_flow[3](quantity = "Power", unit = "W") "component loss-heat flow";
//   protected Real rectifier.rectifier.C0(quantity = "Conductance", unit = "S") = 1.0 "for dq phase system";
//   final parameter Real rectifier.rectifier.par.V_nom(quantity = "ElectricPotential", unit = "V", displayUnit = "kV", min = 0.0) = 100.0 "nom Voltage";
//   final parameter Real rectifier.rectifier.par.I_nom(quantity = "ElectricCurrent", unit = "A", min = 0.0) = 10.0 "nom Current";
//   final parameter Real rectifier.rectifier.par.eps[1](unit = "1", min = 0.0) = 1e-4 "{resistance 'on', conductance 'off'}";
//   final parameter Real rectifier.rectifier.par.eps[2](unit = "1", min = 0.0) = 1e-4 "{resistance 'on', conductance 'off'}";
//   final parameter Real rectifier.rectifier.par.Vf(quantity = "ElectricPotential", unit = "V", displayUnit = "kV", min = 0.0) = 1.0 "forward threshold-voltage";
//   final parameter Real rectifier.rectifier.par.Hsw_nom(quantity = "Energy", unit = "J") = 0.0 "switching loss at V_nom, I_nom (av on off)";
//   final parameter Real rectifier.rectifier.par.T0_loss(quantity = "ThermodynamicTemperature", unit = "K", displayUnit = "degC", min = 0.0, start = 288.15, nominal = 300.0) = 300.0 "reference T for cT_loss expansion";
//   protected Real rectifier.rectifier.V[1](quantity = "Voltage.ThreePhase_dq0", unit = "V", displayUnit = "kV", nominal = 1000.0);
//   protected Real rectifier.rectifier.V[2](quantity = "Voltage.ThreePhase_dq0", unit = "V", displayUnit = "kV", nominal = 1000.0);
//   protected Real rectifier.rectifier.V[3](quantity = "Voltage.ThreePhase_dq0", unit = "V", displayUnit = "kV", nominal = 1000.0);
//   protected Real rectifier.rectifier.v[1](quantity = "Voltage.ThreePhase_dq0", unit = "V", displayUnit = "kV", nominal = 1000.0) "voltage in inertial abc representation";
//   protected Real rectifier.rectifier.v[2](quantity = "Voltage.ThreePhase_dq0", unit = "V", displayUnit = "kV", nominal = 1000.0) "voltage in inertial abc representation";
//   protected Real rectifier.rectifier.v[3](quantity = "Voltage.ThreePhase_dq0", unit = "V", displayUnit = "kV", nominal = 1000.0) "voltage in inertial abc representation";
//   protected Real rectifier.rectifier.i_sc[1](quantity = "Voltage.ThreePhase_dq0", unit = "V", displayUnit = "kV", nominal = 1000.0) "current scaled to voltage in inertial abc representation";
//   protected Real rectifier.rectifier.i_sc[2](quantity = "Voltage.ThreePhase_dq0", unit = "V", displayUnit = "kV", nominal = 1000.0) "current scaled to voltage in inertial abc representation";
//   protected Real rectifier.rectifier.i_sc[3](quantity = "Voltage.ThreePhase_dq0", unit = "V", displayUnit = "kV", nominal = 1000.0) "current scaled to voltage in inertial abc representation";
//   protected Real rectifier.rectifier.s[1](start = 0.5) "arc-length on characteristic";
//   protected Real rectifier.rectifier.s[2](start = 0.5) "arc-length on characteristic";
//   protected Real rectifier.rectifier.s[3](start = 0.5) "arc-length on characteristic";
//   protected Real rectifier.rectifier.switch[1] "switch function in inertial abc representation";
//   protected Real rectifier.rectifier.switch[2] "switch function in inertial abc representation";
//   protected Real rectifier.rectifier.switch[3] "switch function in inertial abc representation";
//   protected Real rectifier.rectifier.Park[1,1];
//   protected Real rectifier.rectifier.Park[1,2];
//   protected Real rectifier.rectifier.Park[1,3];
//   protected Real rectifier.rectifier.Park[2,1];
//   protected Real rectifier.rectifier.Park[2,2];
//   protected Real rectifier.rectifier.Park[2,3];
//   protected Real rectifier.rectifier.Park[3,1];
//   protected Real rectifier.rectifier.Park[3,2];
//   protected Real rectifier.rectifier.Park[3,3];
//   final parameter Boolean meterDC.av = true "time average power";
//   final parameter Real meterDC.tcst(quantity = "Time", unit = "s", min = 1e-9) = 0.1 "average time-constant";
//   Real meterDC.term_p.v[1](quantity = "Voltage.TwoConductor", unit = "V", displayUnit = "kV", nominal = 1000.0) "voltage vector";
//   Real meterDC.term_p.v[2](quantity = "Voltage.TwoConductor", unit = "V", displayUnit = "kV", nominal = 1000.0) "voltage vector";
//   Real meterDC.term_p.i[1](quantity = "Current.TwoConductor", unit = "A", nominal = 1.0) "current vector";
//   Real meterDC.term_p.i[2](quantity = "Current.TwoConductor", unit = "A", nominal = 1.0) "current vector";
//   Real meterDC.term_n.v[1](quantity = "Voltage.TwoConductor", unit = "V", displayUnit = "kV", nominal = 1000.0) "voltage vector";
//   Real meterDC.term_n.v[2](quantity = "Voltage.TwoConductor", unit = "V", displayUnit = "kV", nominal = 1000.0) "voltage vector";
//   Real meterDC.term_n.i[1](quantity = "Current.TwoConductor", unit = "A", nominal = 1.0) "current vector";
//   Real meterDC.term_n.i[2](quantity = "Current.TwoConductor", unit = "A", nominal = 1.0) "current vector";
//   final parameter Boolean meterDC.puUnits = true "= true, if scaled with nom. values (pu), else scaled with 1 (SI)";
//   final parameter Real meterDC.V_nom(quantity = "ElectricPotential", unit = "V", displayUnit = "kV", min = 0.0) = 100.0 "nominal Voltage (= base for pu)";
//   final parameter Real meterDC.S_nom(quantity = "Power", unit = "VA", displayUnit = "MVA", min = 0.0) = 1000.0 "nominal Power (= base for pu)";
//   Real meterDC.p(quantity = "Power", unit = "W/W", stateSelect = StateSelect.never);
//   Real meterDC.p_av(quantity = "Power", unit = "W/W") = meterDC.pav;
//   Real meterDC.v(quantity = "ElectricPotential", unit = "V/V", stateSelect = StateSelect.never);
//   Real meterDC.v0(quantity = "ElectricPotential", unit = "V/V", stateSelect = StateSelect.never);
//   Real meterDC.i(quantity = "ElectricCurrent", unit = "A/A", stateSelect = StateSelect.never);
//   Real meterDC.i0(quantity = "ElectricCurrent", unit = "A/A", stateSelect = StateSelect.never);
//   protected final parameter Real meterDC.V_base(quantity = "Voltage.TwoConductor", unit = "V", displayUnit = "kV", nominal = 1000.0) = 100.0;
//   protected final parameter Real meterDC.I_base(quantity = "Current.TwoConductor", unit = "A", nominal = 1.0) = 10.0;
//   protected Real meterDC.pav(quantity = "Power", unit = "W/W");
//   protected Real meterDC.v_ab[1](quantity = "ElectricPotential", unit = "V/V");
//   protected Real meterDC.v_ab[2](quantity = "ElectricPotential", unit = "V/V");
//   protected Real meterDC.i_ab[1](quantity = "ElectricCurrent", unit = "A/A");
//   protected Real meterDC.i_ab[2](quantity = "ElectricCurrent", unit = "A/A");
//   Real vDC.term.v[1](quantity = "Voltage.TwoConductor", unit = "V", displayUnit = "kV", nominal = 1000.0) "voltage vector";
//   Real vDC.term.v[2](quantity = "Voltage.TwoConductor", unit = "V", displayUnit = "kV", nominal = 1000.0) "voltage vector";
//   Real vDC.term.i[1](quantity = "Current.TwoConductor", unit = "A", nominal = 1.0) "current vector";
//   Real vDC.term.i[2](quantity = "Current.TwoConductor", unit = "A", nominal = 1.0) "current vector";
//   final parameter Boolean vDC.puUnits = true "= true, if scaled with nom. values (pu), else scaled with 1 (SI)";
//   final parameter Real vDC.V_nom(quantity = "ElectricPotential", unit = "V", displayUnit = "kV", min = 0.0) = 100.0 "nominal Voltage (= base for pu)";
//   final parameter Real vDC.S_nom(quantity = "Power", unit = "VA", displayUnit = "MVA", min = 0.0) = 1.0 "nominal Power (= base for pu)";
//   final parameter Integer vDC.pol(min = -1, max = 1) = 0 "grounding scheme";
//   Real vDC.neutral.v(quantity = "ElectricPotential", unit = "V") "Potential at the pin";
//   Real vDC.neutral.i(quantity = "ElectricCurrent", unit = "A") "Current flowing into the pin";
//   protected final parameter Real vDC.V_base = 100.0;
//   final parameter Boolean vDC.use_vDC_in = false "= true to use input signal vDC_in, otherwise use fixed value";
//   protected Real vDC.vDC_internal "Needed to connect to conditional connector";
//   parameter Real vDC.v0(quantity = "ElectricPotential", unit = "V/V") = 1.0 "fixed DC voltage";
//   protected Real vDC.v(quantity = "Voltage.TwoConductor", unit = "V", displayUnit = "kV", nominal = 1000.0);
//   Real grd1.term.v(quantity = "ElectricPotential", unit = "V") "Potential at the pin";
//   Real grd1.term.i(quantity = "ElectricCurrent", unit = "A") "Current flowing into the pin";
//   Real grd2.term.v(quantity = "ElectricPotential", unit = "V") "Potential at the pin";
//   Real grd2.term.i(quantity = "ElectricCurrent", unit = "A") "Current flowing into the pin";
...
// initial equation
//   ind.i[1] = ind.i_start[1];
//   ind.i[2] = ind.i_start[2];
//   ind.i[3] = ind.i_start[3];
//   meterAC.pav[1] = meterAC.p[1];
//   meterAC.pav[2] = meterAC.p[2];
//   meterAC.pav[3] = meterAC.p[3];
//   meterDC.pav = meterDC.p;
// equation
//   vAC.vPhasor_in[1] = vAC.vPhasor_internal[1];
//   vAC.vPhasor_in[2] = vAC.vPhasor_internal[2];
//   rectifier.AC.theta[1] = rectifier.rectifier.AC.theta[1];
//   rectifier.AC.theta[2] = rectifier.rectifier.AC.theta[2];
//   rectifier.AC.v[1] = rectifier.rectifier.AC.v[1];
//   rectifier.AC.v[2] = rectifier.rectifier.AC.v[2];
//   rectifier.AC.v[3] = rectifier.rectifier.AC.v[3];
//   rectifier.rectifier.AC.i[1] - rectifier.AC.i[1] = 0.0;
//   rectifier.rectifier.AC.i[2] - rectifier.AC.i[2] = 0.0;
//   rectifier.rectifier.AC.i[3] - rectifier.AC.i[3] = 0.0;
//   rectifier.rectifier.DC.v[1] = rectifier.DC.v[1];
//   rectifier.rectifier.DC.v[2] = rectifier.DC.v[2];
//   rectifier.rectifier.heat.ports[1].T = rectifier.heat.ports[1].T;
//   rectifier.rectifier.heat.ports[2].T = rectifier.heat.ports[2].T;
//   rectifier.rectifier.heat.ports[3].T = rectifier.heat.ports[3].T;
//   vAC.term.theta[1] = ind.term_p.theta[1];
//   vAC.term.theta[2] = ind.term_p.theta[2];
//   vAC.term.v[1] = ind.term_p.v[1];
//   vAC.term.v[2] = ind.term_p.v[2];
//   vAC.term.v[3] = ind.term_p.v[3];
//   ind.term_n.theta[1] = meterAC.term_p.theta[1];
//   ind.term_n.theta[2] = meterAC.term_p.theta[2];
//   ind.term_n.v[1] = meterAC.term_p.v[1];
//   ind.term_n.v[2] = meterAC.term_p.v[2];
//   ind.term_n.v[3] = meterAC.term_p.v[3];
//   meterAC.term_n.theta[1] = rectifier.AC.theta[1];
//   meterAC.term_n.theta[2] = rectifier.AC.theta[2];
//   meterAC.term_n.v[1] = rectifier.AC.v[1];
//   meterAC.term_n.v[2] = rectifier.AC.v[2];
//   meterAC.term_n.v[3] = rectifier.AC.v[3];
//   rectifier.DC.v[1] = meterDC.term_p.v[1];
//   rectifier.DC.v[2] = meterDC.term_p.v[2];
//   meterDC.term_n.v[1] = vDC.term.v[1];
//   meterDC.term_n.v[2] = vDC.term.v[2];
//   transPh.y[1] = vAC.vPhasor_in[1];
//   transPh.y[2] = vAC.vPhasor_in[2];
//   grd1.term.v = vAC.neutral.v;
//   vDC.neutral.v = grd2.term.v;
//   rectifier.heat.ports[1].T = bdCond.heat.ports[1].T;
//   rectifier.heat.ports[2].T = bdCond.heat.ports[2].T;
//   rectifier.heat.ports[3].T = bdCond.heat.ports[3].T;
//   system.receiveFreq.H = 0.0;
//   system.receiveFreq.w_H = 0.0;
//   ind.term_p.i[1] + vAC.term.i[1] = 0.0;
//   ind.term_p.i[2] + vAC.term.i[2] = 0.0;
//   ind.term_p.i[3] + vAC.term.i[3] = 0.0;
//   meterAC.term_p.i[1] + ind.term_n.i[1] = 0.0;
//   meterAC.term_p.i[2] + ind.term_n.i[2] = 0.0;
//   meterAC.term_p.i[3] + ind.term_n.i[3] = 0.0;
//   rectifier.AC.i[1] + meterAC.term_n.i[1] = 0.0;
//   rectifier.AC.i[2] + meterAC.term_n.i[2] = 0.0;
//   rectifier.AC.i[3] + meterAC.term_n.i[3] = 0.0;
//   meterDC.term_p.i[1] + rectifier.DC.i[1] = 0.0;
//   meterDC.term_p.i[2] + rectifier.DC.i[2] = 0.0;
//   bdCond.heat.ports[1].Q_flow + rectifier.heat.ports[1].Q_flow = 0.0;
//   bdCond.heat.ports[2].Q_flow + rectifier.heat.ports[2].Q_flow = 0.0;
//   bdCond.heat.ports[3].Q_flow + rectifier.heat.ports[3].Q_flow = 0.0;
//   rectifier.rectifier.DC.i[1] - rectifier.DC.i[1] = 0.0;
//   rectifier.rectifier.DC.i[2] - rectifier.DC.i[2] = 0.0;
//   rectifier.rectifier.heat.ports[1].Q_flow - rectifier.heat.ports[1].Q_flow = 0.0;
//   rectifier.rectifier.heat.ports[2].Q_flow - rectifier.heat.ports[2].Q_flow = 0.0;
//   rectifier.rectifier.heat.ports[3].Q_flow - rectifier.heat.ports[3].Q_flow = 0.0;
//   vDC.term.i[1] + meterDC.term_n.i[1] = 0.0;
//   vDC.term.i[2] + meterDC.term_n.i[2] = 0.0;
//   grd2.term.i + vDC.neutral.i = 0.0;
//   grd1.term.i + vAC.neutral.i = 0.0;
//   system.omega = system.omega_internal;
//   when initial() then
//     system.initime = time;
//   end when;
...
//   system.receiveFreq.h = 0.0;
//   system.receiveFreq.w_h = 0.0;
//   transPh.y[1] = 0.5 * (transPh.a_end + transPh.a_start + (transPh.a_end - transPh.a_start) * tanh(transPh.coef * (time - transPh.t_change)));
//   transPh.y[2] = 0.5 * (transPh.ph_end + transPh.ph_start + (transPh.ph_end - transPh.ph_start) * tanh(transPh.coef * (time - transPh.t_change)));
//   vAC.V = vAC.vPhasor_internal[1] * vAC.V_base;
//   vAC.alpha = vAC.vPhasor_internal[2];
//   vAC.phi = vAC.term.theta[1] + vAC.alpha;
//   vAC.term.v = Rectifier_total.vAC.PS.map({vAC.V * cos(vAC.phi), vAC.V * sin(vAC.phi), 1.7320508075688772 * vAC.neutral.v});
//   vAC.omega_internal = 0.0;
//   vAC.theta = system.theta;
//   vAC.term.theta[1] = system.thetaRel;
//   vAC.term.theta[2] = system.thetaRef;
//   1.7320508075688772 * vAC.term.i[3] + vAC.neutral.i = 0.0;
//   Rectifier_total.ind.PS.map({ind.L, ind.L, ind.L0}) .* der(ind.i) + Rectifier_total.ind.j(ind.i) * ind.omega[2] * ind.L + {ind.R * ind.i[1], ind.R * ind.i[2], ind.R * ind.i[3]} = ind.v;
//   ind.omega[1] = der(ind.term_p.theta[1]);
//   ind.omega[2] = der(ind.term_p.theta[2]);
//   ind.v[1] = ind.term_p.v[1] - ind.term_n.v[1];
//   ind.v[2] = ind.term_p.v[2] - ind.term_n.v[2];
//   ind.v[3] = ind.term_p.v[3] - ind.term_n.v[3];
//   ind.i[1] = ind.term_p.i[1];
//   ind.i[2] = ind.term_p.i[2];
//   ind.i[3] = ind.term_p.i[3];
//   ind.term_p.i[1] + ind.term_n.i[1] = 0.0;
//   ind.term_p.i[2] + ind.term_n.i[2] = 0.0;
//   ind.term_p.i[3] + ind.term_n.i[3] = 0.0;
//   ind.term_n.theta[1] = ind.term_p.theta[1];
//   ind.term_n.theta[2] = ind.term_p.theta[2];
//   meterAC.p_av = meterAC.pav;
//   meterAC.v_abc = {meterAC.Park[1,1] * meterAC.v[1] + meterAC.Park[2,1] * meterAC.v[2] + meterAC.Park[3,1] * meterAC.v[3], meterAC.Park[1,2] * meterAC.v[1] + meterAC.Park[2,2] * meterAC.v[2] + meterAC.Park[3,2] * meterAC.v[3], meterAC.Park[1,3] * meterAC.v[1] + meterAC.Park[2,3] * meterAC.v[2] + meterAC.Park[3,3] * meterAC.v[3]};
//   meterAC.vpp_abc = Rectifier_total.meterAC.v2vpp_abc({meterAC.Park[1,1] * meterAC.v[1] + meterAC.Park[2,1] * meterAC.v[2] + meterAC.Park[3,1] * meterAC.v[3], meterAC.Park[1,2] * meterAC.v[1] + meterAC.Park[2,2] * meterAC.v[2] + meterAC.Park[3,2] * meterAC.v[3], meterAC.Park[1,3] * meterAC.v[1] + meterAC.Park[2,3] * meterAC.v[2] + meterAC.Park[3,3] * meterAC.v[3]});
//   meterAC.i_abc = {meterAC.Park[1,1] * meterAC.i[1] + meterAC.Park[2,1] * meterAC.i[2] + meterAC.Park[3,1] * meterAC.i[3], meterAC.Park[1,2] * meterAC.i[1] + meterAC.Park[2,2] * meterAC.i[2] + meterAC.Park[3,2] * meterAC.i[3], meterAC.Park[1,3] * meterAC.i[1] + meterAC.Park[2,3] * meterAC.i[2] + meterAC.Park[3,3] * meterAC.i[3]};
//   meterAC.v[1] = meterAC.term_p.v[1] / meterAC.V_base;
//   meterAC.v[2] = meterAC.term_p.v[2] / meterAC.V_base;
//   meterAC.v[3] = meterAC.term_p.v[3] / meterAC.V_base;
//   meterAC.vpp[1] = 1.7320508075688772 * meterAC.v[2];
//   meterAC.vpp[2] = 1.7320508075688772 * (-meterAC.v[1]);
//   meterAC.i[1] = meterAC.term_p.i[1] / meterAC.I_base;
//   meterAC.i[2] = meterAC.term_p.i[2] / meterAC.I_base;
//   meterAC.i[3] = meterAC.term_p.i[3] / meterAC.I_base;
//   meterAC.p = Rectifier_total.meterAC.PS.phasePowers_vi(meterAC.v, meterAC.i);
//   der(meterAC.pav[1]) = (meterAC.p[1] - meterAC.pav[1]) / 0.1;
//   der(meterAC.pav[2]) = (meterAC.p[2] - meterAC.pav[2]) / 0.1;
//   der(meterAC.pav[3]) = (meterAC.p[3] - meterAC.pav[3]) / 0.1;
//   meterAC.v_norm = sqrt(meterAC.v[1] * meterAC.v[1] + meterAC.v[2] * meterAC.v[2] + meterAC.v[3] * meterAC.v[3]);
//   meterAC.alpha_v = atan2(meterAC.Rot_dq[1,2] * meterAC.v[1] + meterAC.Rot_dq[2,2] * meterAC.v[2], meterAC.Rot_dq[1,1] * meterAC.v[1] + meterAC.Rot_dq[2,1] * meterAC.v[2]);
//   meterAC.i_norm = sqrt(meterAC.i[1] * meterAC.i[1] + meterAC.i[2] * meterAC.i[2] + meterAC.i[3] * meterAC.i[3]);
//   meterAC.alpha_i = atan2(meterAC.Rot_dq[1,2] * meterAC.i[1] + meterAC.Rot_dq[2,2] * meterAC.i[2], meterAC.Rot_dq[1,1] * meterAC.i[1] + meterAC.Rot_dq[2,1] * meterAC.i[2]);
//   meterAC.cos_phi = cos(meterAC.alpha_v - meterAC.alpha_i);
//   meterAC.Park = Rectifier_total.meterAC.park(meterAC.term_p.theta[2]);
//   meterAC.Rot_dq = Rectifier_total.meterAC.rot_dq(meterAC.term_p.theta[1]);
//   meterAC.term_p.v[1] = meterAC.term_n.v[1];
//   meterAC.term_p.v[2] = meterAC.term_n.v[2];
//   meterAC.term_p.v[3] = meterAC.term_n.v[3];
//   meterAC.term_p.i[1] + meterAC.term_n.i[1] = 0.0;
//   meterAC.term_p.i[2] + meterAC.term_n.i[2] = 0.0;
//   meterAC.term_p.i[3] + meterAC.term_n.i[3] = 0.0;
//   meterAC.term_n.theta[1] = meterAC.term_p.theta[1];
//   meterAC.term_n.theta[2] = meterAC.term_p.theta[2];
//   rectifier.rectifier.Park = PowerSystems.Utilities.Transforms.park(rectifier.rectifier.AC.theta[2]);
//   rectifier.rectifier.i_sc[1] = (rectifier.rectifier.Park[1,1] * rectifier.rectifier.AC.i[1] + rectifier.rectifier.Park[2,1] * rectifier.rectifier.AC.i[2] + rectifier.rectifier.Park[3,1] * rectifier.rectifier.AC.i[3]) * 100.0 / 10.0;
//   rectifier.rectifier.i_sc[2] = (rectifier.rectifier.Park[1,2] * rectifier.rectifier.AC.i[1] + rectifier.rectifier.Park[2,2] * rectifier.rectifier.AC.i[2] + rectifier.rectifier.Park[3,2] * rectifier.rectifier.AC.i[3]) * 100.0 / 10.0;
//   rectifier.rectifier.i_sc[3] = (rectifier.rectifier.Park[1,3] * rectifier.rectifier.AC.i[1] + rectifier.rectifier.Park[2,3] * rectifier.rectifier.AC.i[2] + rectifier.rectifier.Park[3,3] * rectifier.rectifier.AC.i[3]) * 100.0 / 10.0;
//   rectifier.rectifier.V[1] = rectifier.rectifier.vDC1 + 1.0;
//   if rectifier.rectifier.s[1] > rectifier.rectifier.V[1] then
//     rectifier.rectifier.v[1] = rectifier.rectifier.par.eps[1] * rectifier.rectifier.s[1] + (1.0 - rectifier.rectifier.par.eps[1]) * rectifier.rectifier.V[1];
//     rectifier.rectifier.i_sc[1] = rectifier.rectifier.s[1] - (1.0 - rectifier.rectifier.par.eps[2]) * rectifier.rectifier.V[1];
//   elseif rectifier.rectifier.s[1] < (-rectifier.rectifier.V[1]) then
//     rectifier.rectifier.v[1] = rectifier.rectifier.par.eps[1] * rectifier.rectifier.s[1] - (1.0 - rectifier.rectifier.par.eps[1]) * rectifier.rectifier.V[1];
//     rectifier.rectifier.i_sc[1] = rectifier.rectifier.s[1] + (1.0 - rectifier.rectifier.par.eps[2]) * rectifier.rectifier.V[1];
//   else
//     rectifier.rectifier.v[1] = rectifier.rectifier.s[1];
//     rectifier.rectifier.i_sc[1] = rectifier.rectifier.par.eps[2] * rectifier.rectifier.s[1];
//   end if;
//   rectifier.rectifier.switch[1] = /*Real*/(noEvent(sign(rectifier.rectifier.s[1])));
//   rectifier.rectifier.V[2] = rectifier.rectifier.vDC1 + 1.0;
//   if rectifier.rectifier.s[2] > rectifier.rectifier.V[2] then
//     rectifier.rectifier.v[2] = rectifier.rectifier.par.eps[1] * rectifier.rectifier.s[2] + (1.0 - rectifier.rectifier.par.eps[1]) * rectifier.rectifier.V[2];
//     rectifier.rectifier.i_sc[2] = rectifier.rectifier.s[2] - (1.0 - rectifier.rectifier.par.eps[2]) * rectifier.rectifier.V[2];
//   elseif rectifier.rectifier.s[2] < (-rectifier.rectifier.V[2]) then
//     rectifier.rectifier.v[2] = rectifier.rectifier.par.eps[1] * rectifier.rectifier.s[2] - (1.0 - rectifier.rectifier.par.eps[1]) * rectifier.rectifier.V[2];
//     rectifier.rectifier.i_sc[2] = rectifier.rectifier.s[2] + (1.0 - rectifier.rectifier.par.eps[2]) * rectifier.rectifier.V[2];
//   else
//     rectifier.rectifier.v[2] = rectifier.rectifier.s[2];
//     rectifier.rectifier.i_sc[2] = rectifier.rectifier.par.eps[2] * rectifier.rectifier.s[2];
//   end if;
//   rectifier.rectifier.switch[2] = /*Real*/(noEvent(sign(rectifier.rectifier.s[2])));
//   rectifier.rectifier.V[3] = rectifier.rectifier.vDC1 + 1.0;
//   if rectifier.rectifier.s[3] > rectifier.rectifier.V[3] then
//     rectifier.rectifier.v[3] = rectifier.rectifier.par.eps[1] * rectifier.rectifier.s[3] + (1.0 - rectifier.rectifier.par.eps[1]) * rectifier.rectifier.V[3];
//     rectifier.rectifier.i_sc[3] = rectifier.rectifier.s[3] - (1.0 - rectifier.rectifier.par.eps[2]) * rectifier.rectifier.V[3];
//   elseif rectifier.rectifier.s[3] < (-rectifier.rectifier.V[3]) then
//     rectifier.rectifier.v[3] = rectifier.rectifier.par.eps[1] * rectifier.rectifier.s[3] - (1.0 - rectifier.rectifier.par.eps[1]) * rectifier.rectifier.V[3];
//     rectifier.rectifier.i_sc[3] = rectifier.rectifier.s[3] + (1.0 - rectifier.rectifier.par.eps[2]) * rectifier.rectifier.V[3];
//   else
//     rectifier.rectifier.v[3] = rectifier.rectifier.s[3];
//     rectifier.rectifier.i_sc[3] = rectifier.rectifier.par.eps[2] * rectifier.rectifier.s[3];
//   end if;
//   rectifier.rectifier.switch[3] = /*Real*/(noEvent(sign(rectifier.rectifier.s[3])));
//   rectifier.rectifier.v_dq0[1] = rectifier.rectifier.Park[1,1] * rectifier.rectifier.v[1] + rectifier.rectifier.Park[1,2] * rectifier.rectifier.v[2] + rectifier.rectifier.Park[1,3] * rectifier.rectifier.v[3];
//   rectifier.rectifier.v_dq0[2] = rectifier.rectifier.Park[2,1] * rectifier.rectifier.v[1] + rectifier.rectifier.Park[2,2] * rectifier.rectifier.v[2] + rectifier.rectifier.Park[2,3] * rectifier.rectifier.v[3];
//   rectifier.rectifier.v_dq0[3] = rectifier.rectifier.Park[3,1] * rectifier.rectifier.v[1] + rectifier.rectifier.Park[3,2] * rectifier.rectifier.v[2] + rectifier.rectifier.Park[3,3] * rectifier.rectifier.v[3];
//   rectifier.rectifier.switch_dq0[1] = rectifier.rectifier.Park[1,1] * rectifier.rectifier.switch[1] + rectifier.rectifier.Park[1,2] * rectifier.rectifier.switch[2] + rectifier.rectifier.Park[1,3] * rectifier.rectifier.switch[3];
//   rectifier.rectifier.switch_dq0[2] = rectifier.rectifier.Park[2,1] * rectifier.rectifier.switch[1] + rectifier.rectifier.Park[2,2] * rectifier.rectifier.switch[2] + rectifier.rectifier.Park[2,3] * rectifier.rectifier.switch[3];
//   rectifier.rectifier.switch_dq0[3] = rectifier.rectifier.Park[3,1] * rectifier.rectifier.switch[1] + rectifier.rectifier.Park[3,2] * rectifier.rectifier.switch[2] + rectifier.rectifier.Park[3,3] * rectifier.rectifier.switch[3];
//   rectifier.rectifier.Q_flow[1] = (rectifier.rectifier.v[1] - rectifier.rectifier.switch[1] * rectifier.rectifier.vDC1) * rectifier.rectifier.i_sc[1] * 10.0 / 100.0;
//   rectifier.rectifier.Q_flow[2] = (rectifier.rectifier.v[2] - rectifier.rectifier.switch[2] * rectifier.rectifier.vDC1) * rectifier.rectifier.i_sc[2] * 10.0 / 100.0;
//   rectifier.rectifier.Q_flow[3] = (rectifier.rectifier.v[3] - rectifier.rectifier.switch[3] * rectifier.rectifier.vDC1) * rectifier.rectifier.i_sc[3] * 10.0 / 100.0;
//   rectifier.rectifier.AC.v = rectifier.rectifier.v_dq0 + Rectifier_total.rectifier.rectifier.PS.map({0.0, 0.0, 1.7320508075688772 * rectifier.rectifier.vDC0});
//   rectifier.rectifier.iDC1 + rectifier.rectifier.switch_dq0[1] * rectifier.rectifier.AC.i[1] + rectifier.rectifier.switch_dq0[2] * rectifier.rectifier.AC.i[2] + rectifier.rectifier.switch_dq0[3] * rectifier.rectifier.AC.i[3] = 0.0;
//   rectifier.rectifier.iDC0 + 1.7320508075688772 * rectifier.rectifier.AC.i[3] = 0.0;
//   rectifier.rectifier.T[1] = rectifier.rectifier.heat.ports[1].T;
//   rectifier.rectifier.T[2] = rectifier.rectifier.heat.ports[2].T;
//   rectifier.rectifier.T[3] = rectifier.rectifier.heat.ports[3].T;
//   rectifier.rectifier.heat.ports[1].Q_flow = -rectifier.rectifier.Q_flow[1];
//   rectifier.rectifier.heat.ports[2].Q_flow = -rectifier.rectifier.Q_flow[2];
//   rectifier.rectifier.heat.ports[3].Q_flow = -rectifier.rectifier.Q_flow[3];
//   meterDC.v_ab[1] = meterDC.term_p.v[1] / meterDC.V_base;
//   meterDC.v_ab[2] = meterDC.term_p.v[2] / meterDC.V_base;
//   meterDC.i_ab[1] = meterDC.term_p.i[1] / meterDC.I_base;
//   meterDC.i_ab[2] = meterDC.term_p.i[2] / meterDC.I_base;
//   meterDC.v = meterDC.v_ab[1] - meterDC.v_ab[2];
//   meterDC.v0 = (meterDC.v_ab[1] + meterDC.v_ab[2]) / 2.0;
//   meterDC.i = (meterDC.i_ab[1] - meterDC.i_ab[2]) / 2.0;
//   meterDC.i0 = meterDC.i_ab[1] + meterDC.i_ab[2];
//   meterDC.p = meterDC.v_ab[1] * meterDC.i_ab[1] + meterDC.v_ab[2] * meterDC.i_ab[2];
//   der(meterDC.pav) = (meterDC.p - meterDC.pav) / 0.1;
//   meterDC.term_p.v[1] = meterDC.term_n.v[1];
//   meterDC.term_p.v[2] = meterDC.term_n.v[2];
//   meterDC.term_p.i[1] + meterDC.term_n.i[1] = 0.0;
//   meterDC.term_p.i[2] + meterDC.term_n.i[2] = 0.0;
//   vDC.vDC_internal = vDC.v0;
//   vDC.v = vDC.vDC_internal * vDC.V_base;
//   vDC.term.v[1] - vDC.term.v[2] = vDC.v;
//   vDC.term.v[1] + vDC.term.v[2] = vDC.neutral.v;
//   vDC.term.i[1] + vDC.term.i[2] + vDC.neutral.i = 0.0;
//   grd1.term.v = 0.0;
//   grd2.term.v = 0.0;
