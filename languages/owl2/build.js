...
if (isParserUpToDate()) {
  console.log("[owl2] WebAssembly parser is up to date, skipping GLR & asc build. (use --force to rebuild)");
} else {
...
const languagePath = path.join(__dirname, "src", "language.ts");
const result = buildParser(owl2Language, { sourcePath: languagePath });
...
console.log("[owl2] WebAssembly parser built successfully -> " + outWasm);
