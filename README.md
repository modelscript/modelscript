...
- **Arena Data-Oriented Simulator**: The simulator utilizes a zero-garbage, Data-Oriented Design (DoD) architecture (`DAEBuilder`). By moving from legacy recursive AST visitors to direct flat-buffer indexing, it provides high-performance equation sorting, Pantelides index reduction, and bipartite matching with zero allocation overhead.
