...
export async function compileDslToWasm(
  ...
  const { error } = await asc.main(flags, {
    ...
    readFile(filename: string) {
      ...
      if (vfs[`${filename}.ts`]) return vfs[`${filename}.ts`];
      if (vfs[normalized]) return vfs[normalized];
      if (vfs[`${normalized}.ts`]) return vfs[`${normalized}.ts`];
      const baseName = normalized.replace(/^.*[/\\]/, "");
      if (vfs[baseName]) return vfs[baseName];
      if (vfs[`${baseName}.ts`]) return vfs[`${baseName}.ts`];
      return null;
    },
