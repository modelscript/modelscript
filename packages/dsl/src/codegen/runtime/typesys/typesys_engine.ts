...
// --- Semantic Analysis & Hindley-Milner Type System Engine ---
// High-performance Type Arena and Union-Find in WebAssembly Linear Memory.
...
// Diagnostic and external graph resolution hooks (mocked during check:as, linked at runtime)
@external("env", "lsp_allocDiagnostic")
export declare function lsp_allocDiagnostic(startByte: u32, endByte: u32, code: u32, data: u32): void;

@external("env", "resolveFqnSymbol")
export declare function resolveFqnSymbol(targetNamePtr: u32): u32;
...
export let unifyErrorNodes = createChunkedUint32Array(10000);
export function getUnifyErrorCount(): u32 {
...
export function getTypeKind(typeId: u32): u16 {
  return (typeTable[typeId * TYPE_STRIDE] & 0xffff) as u16;
}
export function getTypeFlags(typeId: u32): u16 {
  return (typeTable[typeId * TYPE_STRIDE] >>> 16) as u16;
}
export function getTypeSize(typeId: u32): u32 {
  return typeTable[typeId * TYPE_STRIDE + 1];
}
export function getTypeBase(typeId: u32): u32 {
  return typeTable[typeId * TYPE_STRIDE + 2];
}
export function getTypeExtra(typeId: u32): u32 {
  return typeTable[typeId * TYPE_STRIDE + 3];
}
export function getTypeCtorTag(typeId: u32): u32 {
  return typeTable[typeId * TYPE_STRIDE + 4];
}
export function getTypeCtorArg2(typeId: u32): u32 {
  return typeTable[typeId * TYPE_STRIDE + 5];
}
...
export function occursIn(varId: u32, typeId: u32): boolean {
...
export function unify(t1: u32, t2: u32): boolean {
  ...
  if (ctor1 != CTOR_NONE && ctor2 != CTOR_NONE) {
    if (ctor1 != ctor2) {
      unifyErrorCount++;
      return false; // Constructor mismatch
    }
...
export function findType(i: u32): u32 {
  return typeFind(i);
}
...
export function getSymbolKind(id: u32): u16 {
  return (symbolTable[id * SYMBOL_STRIDE] & 0xffff) as u16;
}
export function getSymbolFlags(id: u32): u16 {
  return (symbolTable[id * SYMBOL_STRIDE] >>> 16) as u16;
}
export function getSymbolAstNode(id: u32): u32 {
  return symbolTable[id * SYMBOL_STRIDE + 1];
}
export function getSymbolParent(id: u32): u32 {
  return symbolTable[id * SYMBOL_STRIDE + 2];
}
export function getSymbolFirstChild(id: u32): u32 {
  return symbolTable[id * SYMBOL_STRIDE + 3];
}
export function getSymbolNextSibling(id: u32): u32 {
  return symbolTable[id * SYMBOL_STRIDE + 4];
}
export function getSymbolName(id: u32): u32 {
  return symbolTable[id * SYMBOL_STRIDE + 5];
}
export function getSymbolType(id: u32): u32 {
  return symbolTable[id * SYMBOL_STRIDE + 6];
}
export function getSymbolArgs(id: u32): u32 {
  return symbolTable[id * SYMBOL_STRIDE + 7];
}
...
export function env_specialize(baseClassSymbol: u32, argsPtr: u32): u32 {
  return allocSymbol(
    SYMBOL_KIND_VIRTUAL_SPECIALIZATION,
    0,
    getSymbolAstNode(baseClassSymbol),
    baseClassSymbol,
    getSymbolName(baseClassSymbol),
    getSymbolType(baseClassSymbol),
    argsPtr,
  );
}
...
export function type_builder_sparse_tensor(baseTypeId: u32, shapeId: u32, format: u32): u32 {
  for (let i = typeCount - 1; i >= 1; i--) {
    if (
      getTypeKind(i) == TYPE_SPARSE_TENSOR &&
      getTypeBase(i) == baseTypeId &&
      getTypeExtra(i) == shapeId &&
      getTypeSize(i) == format
    ) {
      return i;
    }
  }
...
export function type_builder_record_field(nameHash: u32, fieldType: u32, nextField: u32): u32 {
  for (let i = typeCount - 1; i >= 1; i--) {
    if (
      getTypeKind(i) == TYPE_RECORD_FIELD &&
      getTypeExtra(i) == nameHash &&
      getTypeBase(i) == fieldType &&
      getTypeCtorArg2(i) == nextField
    ) {
      return i;
    }
  }
  let ptr = allocType(TYPE_RECORD_FIELD, 0, fieldType, nameHash);
  typeTable[ptr * TYPE_STRIDE + 5] = nextField;
  return ptr;
}
...
export let nodeTypeVars = createChunkedUint32Array(100000);
...
export function resolveUFForDisplay(typeVar: u32): u32 {
  ...
  if (kind != TYPE_VARIABLE) return root;
...
export function evaluateType(typeId: u32, scopeId: u32 = 0): u32 {
  ...
  if (getTypeKind(typeId) == 12 /* TYPE_CONDITIONAL */) {
    ...
    let checkType = typeTable[condPtr * TYPE_STRIDE];
    let extendsType = typeTable[condPtr * TYPE_STRIDE + 1];
