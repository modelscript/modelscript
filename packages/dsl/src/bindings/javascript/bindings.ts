  ...
  getDiagnostics(astRoot: number, rangeStart: number = 0, rangeEnd: number = 0): Diagnostic[] {
    ...
    for (let i = 0; i < numElements * 7; i += 7) {
      ...
      if (rawLintId === 0x7ffe) {
        ...
        const formatSyntaxTokenName = (rawId: number, fallbackName?: string): string => {
          let name =
            fallbackName ??
            ((this.syntaxNames && this.syntaxNames[rawId]) ||
              (rawId >= 32 && rawId <= 126 ? String.fromCharCode(rawId) : rawId > 0 ? `token_${rawId}` : ""));
        ...
        if (arg0 === 1 && rawArg1 > 0) {
          ...
          let rawArg2 = (arg2 & 0x7fff) as number;
          let expName1 = rawArg2 > 0 ? formatSyntaxTokenName(rawArg2) : "";
          let symName = extracted || (rawArg1 > 0 ? formatSyntaxTokenName(rawArg1) : "");
      ...
      if (rawLintId > 0) {
        ...
        if (lintId < 0x8000 && key !== null) {
          ...
          if (typeof msgVal === "function") {
            ...
            const getSyntaxNodeForPtr = (ptr: number): SyntaxNode | null => {
              const isAligned = ptr > 0 && ptr % 4 === 0 && ptr / 4 < memory.length - 4;
              ...
              return new SyntaxNode(dummyTree as any, ptr, actualStart, null, 0, len, typeId);
            };
            ...
            const createContext = (nodePtr: number, fallbackText: string) => {
              ...
              const resolveChildFieldText = (fieldName: string): string => {
                if (syntaxNode) {
              ...
              return new Proxy(
                {},
                {
                  get: (target, prop: string | symbol) => {
                    ...
                    if (prop === "field") return (name: string) => resolveChildFieldText(name);
                    ...
                    if (prop === "fields") {
                      return new Proxy(
                        {},
                        {
                          get: (_, fieldName: string) => resolveChildFieldText(fieldName),
                        },
                      );
                    }
...
export class SyntaxNode {
  ...
  private _cachedHasError: boolean | undefined = undefined;
