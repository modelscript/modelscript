...
function generateFmi2FunctionsC(
  ...
  if (dae.stateMachines.length > 0) {
    ...
    for (let si = 0; si < dae.stateMachines.length; si++) {
      ...
      // Collect transitions from equations
      ...
      if (transitions.length > 0) {
        ...
        for (const tr of transitions) {
          ...
          first = false;
          // Simplified: transition fires when active state matches `from` AND condition is true
          // The condition expression would need full C expression emission which is complex;
          // for now we emit a comment placeholder with the fromState → toState mapping
          lines.push(
