...
import { ArenaSimulator, ModelicaCalibrator } from "@modelscript/compiler/optimizer";
import {
...
export function evaluateCalibrate(
  ...
  try {
    const simulator = new ArenaSimulator(dae.arena);
    simulator.prepare();

    const calibrator = new ModelicaCalibrator(dae.arena, simulator, {
