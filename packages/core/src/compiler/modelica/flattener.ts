...
const _boolCache = new Map<boolean, ModelicaBooleanLiteral>();
const _stringCache = new Map<string, ModelicaStringLiteral>();
const _nameCache = new Map<string, ModelicaNameExpression>();

const _realExprCache = new Map<string, any>();
const _intExprCache = new Map<string, any>();

export const AstBuilder = {
  int: (value: number, text?: string) => {
    if (text) return new ModelicaIntegerLiteral(value, text);
    let cached = _intCache.get(value);
    if (!cached) {
      cached = new ModelicaIntegerLiteral(value);
      _intCache.set(value, cached);
    }
    return cached;
  },
  real: (value: number, text?: string) => {
    if (text) return new ModelicaRealLiteral(value, text);
    let cached = _realCache.get(value);
    if (!cached) {
      cached = new ModelicaRealLiteral(value);
      _realCache.set(value, cached);
    }
    return cached;
  },
  bool: (value: boolean) => {
    let cached = _boolCache.get(value);
    if (!cached) {
      cached = new ModelicaBooleanLiteral(value);
      _boolCache.set(value, cached);
    }
    return cached;
  },
  string: (value: string) => {
    let cached = _stringCache.get(value);
    if (!cached) {
      cached = new ModelicaStringLiteral(value);
      _stringCache.set(value, cached);
    }
    return cached;
  },
  name: (name: string) => {
    ...
    if (!cached) {
      cached = new ModelicaNameExpression(name);
          ...
          const tryExpandToArray = (expr: ModelicaExpression): ModelicaArray | null => {
            ...
            if (expr instanceof ModelicaSubscriptedExpression && expr.base instanceof ModelicaNameExpression) {
              ...
              if (expr.subscripts.length === 1 && expr.subscripts[0] instanceof ModelicaColonExpression) {
                const vars = [...ctx.dae.arenaVariables()]
  ...
  [
    ...
    (args, ctx) => {
      ...
      if (
        ...
        if (innerFuncDef) {
          const outputVars = innerFuncDef.legacyVariables.filter((v) => v.causality === "output");
            ...
            if (nameMatch?.[1]) {
              ...
              if (dimExpr) {
                ...
                if (sizeMatch?.[1] && sizeMatch[2]) {
                  ...
                  const inputVars = innerFuncDef.legacyVariables.filter((v) => v.causality === "input");
      ...
      if (
        ...
        const funcVar = ctx.dae.arenaGetEncoded(varName);
      ...
      if (arrayArg instanceof ModelicaNameExpression && ctx.dae) {
        const matchingVars = ctx.dae.arenaGetArrayElements(arrayArg.name);
 ...
 */
function getUnderlyingPredefinedClass(
  ...
  if (
    isModelicaPredefined ||
    (cls instanceof ModelicaPredefinedClassInstance && !(cls instanceof ModelicaEnumerationClassInstance))
  )
    return cls as any;
  ...
  if ("extendsClassInstances" in cls) {
    for (const ext of (cls as any).extendsClassInstances) {
      if (ext.classInstance) {
        const base = getUnderlyingPredefinedClass(ext.classInstance, visited);
        if (base) return base;
      }
    }
  }
  // Fallback: check declaredElements for legacy ModelicaExtendsClassInstance
  const declaredEls = "declaredElements" in cls ? (cls as any).declaredElements : (cls as any).elements;
  for (const el of declaredEls) {
    if (el instanceof ModelicaExtendsClassInstance && el.classInstance) {
      const base = getUnderlyingPredefinedClass(el.classInstance, visited);
      if (base) return base;
    }
  }
  return null;
...
export class ModelicaFlattener extends ModelicaModelVisitor<[string, ModelicaDAE]> {
   ...
   */
  visitArrayClassInstance(node: any /* ModelicaArrayClassInstance */, args: [string, ModelicaDAE]): void {
    this.visitClassInstance(node as any, args);
  }
   ...
   * @param args - A tuple of `[prefixString, activeDAE]`.
   */
  visitEntity(node: any /* ModelicaEntity */, args: [string, ModelicaDAE]): void {
    this.visitClassInstance(node as any, args);
  }
  ...
  static getMergedElements(node: any): any[] {
    ...
    for (const element of node.elements) {
      // Deduplicate by name: the instantiate query may yield the same component
      // through multiple extends paths (e.g. port_a appearing 8× instead of once).
      // Each path produces a distinct specialized ID, so we dedup by name instead.
      if (element instanceof ModelicaComponentInstance) {
  ...
  visitClassInstance(node: any /* ModelicaClassInstance */, args: [string, ModelicaDAE]): void {
        ...
        for (const modArg of classMod.modificationArguments) {
          ...
          if (
            ...
            if ((modName === "startTime" || modName === "finalTime") && flattened instanceof ModelicaIntegerLiteral) {
              flattened = AstBuilder.real(flattened.value);
            }
    ...
    if ("shortClassTarget" in node && (node as any).shortClassTarget) {
      ...
      if (classMod && classMod.modificationArguments) {
        // Track the merged modifications
    ...
    if (classMod) {
      for (const modArg of classMod.modificationArguments ?? []) {
        if (modArg.ast?.redeclare || modArg.arg?.redeclare || modArg.isRedeclare) {
          ...
          } else if (modArg.ast?.shortClassDefinition) {
            const typeName =
              modArg.ast.shortClassDefinition.identifier?.text ??
              modArg.ast.shortClassDefinition.classSpecifier?.identifier?.text;
            const typeParts = modArg.ast.shortClassDefinition.classSpecifier?.typeSpecifier?.name?.parts?.map(
              (p: any) => p.text,
            );
    ...
    for (const ext of extendsElements) {
      ...
      if (!extMod && (ext as any).abstractSyntaxNode?.classOrInheritanceModification) {
        ...
        extMod = {
          get modificationArguments() {
            const classMod =
              astMod.classModification ??
              astMod.modification?.classModification ??
              astMod.annotationClause?.classModification;
            let args: any[] = [];
            if (classMod) {
              args = classMod.modificationArguments ?? [];
            } else if (astMod.modificationArgumentOrInheritanceModifications) {
              args = astMod.modificationArgumentOrInheritanceModifications;
            }
            return args.map((a: any) => ({
              get name() {
                return (
                  a.name?.parts?.map((p: any) => p.text).join(".") ??
                  a.identifier?.text ??
                  a.componentReference?.parts?.map((p: any) => p.text).join(".") ??
                  null
                );
              },
              get modification() {
          ...
          get modificationExpression() {
            const modExpr = astMod.modificationExpression ?? astMod.modification?.modificationExpression;
      ...
      if (extMod && extMod.modificationArguments) {
        for (const el of activeElements) {
          if (el instanceof ModelicaComponentInstance) {
            const modArg = extMod.modificationArguments.find(
              (m: any) => m.name === el.name || m.name?.text === el.name || m.name?.parts?.[0]?.text === el.name,
            );
            if (modArg) {
              const newMod = modArg.modification || modArg;
      ...
      if (element instanceof ModelicaComponentInstance) {
        ...
        if (element.classInstance instanceof ModelicaArrayClassInstance) {
          ...
          if (bindingExpr) {
            ...
            if (element.classInstance.shape.some((d: any) => d === 0)) {
    ...
    if (t_comp_end - t_comp_start > 1000)
      console.log(
        `[Flattener] visitClassInstance components loop took ${t_comp_end - t_comp_start}ms for ${activeElements.length} elements in ${node.name}`,
      );
    ...
    if (this.#flowConnectPairs.length > 0) {
      ...
      for (const [, flowVars] of connectionSets) {
        ...
        let sum: ModelicaExpression = AstBuilder.name(firstName);
    ...
    this.#redeclareContext = savedRedeclare;
    ...
    if (this.activeClassStack.length === 0) {
      ...
      for (const v of args[1].legacyVariables) varByName.set(v.name, v);
      let changed = true;
      while (changed) {
        changed = false;
        for (const v of args[1].legacyVariables) {
  ...
  static mergeModifications(baseMod: any, newMod: any): any {
    ...
    const baseArgs = baseMod.modificationArguments || [];
    for (const m of baseArgs) {
      const name = m.name?.text ?? m.name?.parts?.[0]?.text ?? m.name ?? m.arg?.name;
      if (name) argsMap.set(name, m);
    }

    const newArgs = newMod.modificationArguments || [];
    for (const m of newArgs) {
      const name = m.name?.text ?? m.name?.parts?.[0]?.text ?? m.name ?? m.arg?.name;
      if (!name) continue;
      const existing = argsMap.get(name);
      if (existing && m.modification && existing.modification) {
        argsMap.set(name, {
          ...m,
    ...
    return {
      ...newMod,
      modificationArguments: Array.from(argsMap.values()),
      modificationExpression: newMod.modificationExpression || baseMod.modificationExpression,
      expression: newMod.expression || baseMod.expression,
      getModificationArgument: (name: string) => argsMap.get(name),
    };
  }
  ...
  visitComponentInstance(node: any /* ModelicaComponentInstance */, args: [string, ModelicaDAE]): void {
    ...
    if (compMod) {
      const parentClass = this.activeClassStack[this.activeClassStack.length - 1];
      for (const modArg of compMod.modificationArguments ?? []) {
        ...
        if (isRedeclare && parentClass) {
          if (modArg.ast?.componentClause) {
            const componentName = modArg.ast.componentClause.componentDeclaration?.declaration?.identifier?.text;
            const typeParts = modArg.ast.componentClause.typeSpecifier?.name?.parts?.map((p: any) => p.text);
            if (componentName && typeParts && typeParts.length > 0) {
              const targetClass = parentClass.resolveName ? parentClass.resolveName(typeParts) : null;
              if (targetClass) {
                this.#redeclareContext.set(componentName, targetClass);
              }
            }
        ...
        } else if (!isRedeclare && modArg.name) {
          let parts: string[] = [];
    ...
    const predefinedBase = getUnderlyingPredefinedClass(classInstance);
    if (predefinedBase) {
      this.#flattenPredefinedClass(node, name, args, effectiveVariability, currentMods);
        ...
        if (annClause.classModification?.modificationArguments) {
          ...
          if ((cadAnn && cadAnn.modification) || (cadPortAnn && cadPortAnn.modification)) {
            ...
            if (!this.#emittedVarNames.has(dummyVar.name)) {
              ...
              args[1].legacyVariables.push(dummyVar);
            }
          }
        }
      }
      ...
      // If this component was redeclared, its visibility overrides the original declaration
      // We know it was redeclared if its modification is a MergedModification or if it was marked as redeclare
      // But we can just use the component's own isProtected flag which is updated during redeclaration
      // The issue is `this.#outerProtected` might force it to be protected if the outer class is protected,
      // but for RedeclareVisibility.mo, the component itself was originally protected, and redeclared public.
      // Actually, if the redeclaration is public, it should override.
      let effectiveProtected = node.isProtected;
      // If node was redeclared via extends, the new modifier visibility should override.
      if (node.isRedeclare || (node.modification && node.modification.constructor?.name === "ModelicaModification")) {
        // If it's redeclared, check if the redeclaration explicitly set visibility.
        // For now, if it was redeclared and the new decl isn't protected, it's public.
        // The AST should have the correct visibility.
        if (node.abstractSyntaxNode?.visibility !== undefined) {
          effectiveProtected = node.abstractSyntaxNode.visibility === ModelicaVisibility.PROTECTED;
        } else if (node.modification?.visibility !== undefined) {
          effectiveProtected = node.modification.visibility === ModelicaVisibility.PROTECTED;
        } else {
          effectiveProtected = false; // default to public if redeclared without protected
        }
      }
      this.#outerProtected = this.#outerProtected || effectiveProtected;
      ...
      const startVars = args[1].legacyVariables.length;
      const startEqs = args[1].equations.length;

      const savedRedeclare = new Map(this.#redeclareContext);

      this.#inheritedModificationsStack.push(currentMods);
      ...
      const endVars = args[1].legacyVariables.length;
      const endEqs = args[1].equations.length;
      this.#componentContents.set(name, {
        variables: args[1].legacyVariables.slice(startVars, endVars),
  ...
  #getEvaluationScope(
    ...
    for (let i = this.activeClassStack.length - 1; i >= 0; i--) {
      ...
      if (
        ...
        isSameScope((ancestorClass as any).shortClassTarget, modScope)
      ) {
        match = true;
  ...
  #flattenToSymbolic(
    ...
    if (typeof expr.accept === "function") {
      ...
      if (result == null && expr.kind === "expression" && expr.text && this.context) {
        try {
          let tree = _realExprCache.get(expr.text);
          if (!tree) {
            tree = this.context.parse(".mo", `model Dummy Real x = ${expr.text}; end Dummy;`);
            _realExprCache.set(expr.text, tree);
          }
          const modExpr = tree.rootNode
            .childForFieldName("classDefinition")
            ?.childForFieldName("classSpecifier")
            ?.childForFieldName("section")
            ?.firstNamedChild // ComponentClause
            ?.childForFieldName("componentDeclaration")
            ?.childForFieldName("declaration")
            ?.childForFieldName("modification")
            ?.childForFieldName("modificationExpression");

          const exprNode = modExpr?.childForFieldName("expression") ?? modExpr?.firstNamedChild;
          if (exprNode) {
            const astExpr = ModelicaSyntaxNode.new(null, exprNode);

            if (astExpr) {
              result = astExpr.accept(new ModelicaSyntaxFlattener(this.options), {
                prefix: evalScope.prefix,
                classInstance: evalScope.classInstance,
                dae: dae,
                stmtCollector: [],
      ...
      if (expr instanceof ModelicaUnsignedIntegerLiteralSyntaxNode) {
        return AstBuilder.int(expr.value);
      }
      if (expr instanceof ModelicaUnsignedRealLiteralSyntaxNode) {
        return AstBuilder.real(expr.value, expr.text ?? undefined);
      }
      if (expr instanceof ModelicaComponentReferenceSyntaxNode) {
        ...
        if (parts && parts.length > 0) {
          const name = parts
            ...
            .filter(Boolean)
            .join(".");
          if (name) return AstBuilder.name(prefix ? `${prefix}.${name}` : name);
        }
      }
  ...
  /**
   * Attempts to constant-evaluate an AST node at compile-time using the interpreter.
   * If successful and the result is literal or an array of literals, it returns the evaluated ModelicaExpression.
   * Extracts the first element of a returned tuple for multi-output functions unless `keepTuple` is true.
   */
  #tryEvaluateExpression(expr: any, ctxNode: ModelicaClassInstance, keepTuple = false): ModelicaExpression | null {
    if (!expr || typeof expr.accept !== "function") return null;
    try {
      const interp = new ModelicaInterpreter(true);
      const result = expr.accept(interp, ctxNode);
      if (result && typeof result.accept === "function") {
        if (!keepTuple && result instanceof ModelicaArray && expr.functionReference) {
          // If the interpreter returns an array for a function call, check if it's a multi-output tuple
          const resolved = ctxNode.resolveComponentReference?.(expr.functionReference);
          if (resolved instanceof ModelicaClassInstance && resolved.classKind === ModelicaClassKind.FUNCTION) {
            const outputs = Array.from(resolved.outputParameters);
            if (outputs.length > 1) {
              return result.elements[0] ?? result;
            }
          }
        }
        return result;
      ...
      // Ignore evaluation errors (e.g. unresolved symbols) and fall back to symbolic IR
    }
    return null;
  }
  ...
  #flattenPredefinedClass(
    ...
    if (node.isRedeclare || (node.modification && node.modification.constructor?.name === "ModelicaModification")) {
      if (node.abstractSyntaxNode?.visibility !== undefined) {
        isProtected = node.abstractSyntaxNode.visibility === ModelicaVisibility.PROTECTED;
      } else if (node.modification?.visibility !== undefined) {
        isProtected = node.modification.visibility === ModelicaVisibility.PROTECTED;
      } else {
        isProtected = false; // default to public if redeclared without protected
      }
    ...
    this.#collectTypeAttributes(classInst, attributes);
    ...
    if (variability === ModelicaVariability.CONSTANT) {
      ...
      if (!rawExpr && dynamicModExpr) {
        rawExpr = dynamicModExpr;
      }
      if (!rawExpr) {
    ...
    if (
      ...
      if (bindingExpr instanceof ModelicaFunctionCallSyntaxNode) {
        ...
        if (funcRef) {
          const resolved = node.parent?.resolveComponentReference(funcRef as any);
    ...
    const predefinedBase = getUnderlyingPredefinedClass(node.classInstance);

    if (predefinedBase?.name === "Boolean") {
    ...
    } else if (predefinedBase?.name === "Clock") {
    ...
    } else if (predefinedBase?.name === "Integer") {
    ...
    } else if (predefinedBase?.name === "Real") {
      for (const key of ["start", "min", "max", "nominal"]) {
        if (attributes.has(key)) {
          const casted = castToReal((attributes.get(key) as ModelicaExpression | undefined) ?? null);
      ...
      if (
        ...
        if (resolved instanceof ModelicaClassInstance) {
          for (const comp of resolved.components) {
            if (comp.causality === "output" && getUnderlyingPredefinedClass(comp.classInstance)?.name === "Integer") {
              realBinding = AstBuilder.call("/*Real*/", [realBinding]);
              break;
            }
          }
        }
    ...
    } else if (predefinedBase?.name === "String") {
    ...
    } else if (predefinedBase?.name === "Expression") {
    ...
    if (variable && isFinal) {
      ...
      if (modExpr instanceof ModelicaComponentReferenceSyntaxNode) {
        ...
        if (parts && parts.length > 0) {
          ...
          const targetVar = args[1].legacyVariables.find((v) => v.name === targetName);
    ...
    if (variable) {
      ...
      if (!this.#emittedVarNames.has(variable.name)) {
        ...
        args[1].legacyVariables.push(variable);
  ...
  #flattenEnumerationClass(node: ModelicaComponentInstance, name: string, args: [string, ModelicaDAE]): void {
    ...
    const literals = (node.classInstance as ModelicaEnumerationClassInstance).enumerationLiterals ?? [];
  ...
  #flattenArrayClass(
    ...
    const rawASTExpr = rawArrayBinding;
    let usedSyntaxFlattener = false;

    // Attempt to constant-evaluate array-returning functions even if they aren't marked as parameters
    if ((!arrayBindingExpression || hasMalformedBinding || !isCompileTimeEvaluableEarly) && rawASTExpr) {
      if (rawASTExpr instanceof ModelicaFunctionCallSyntaxNode) {
        const evalResult = this.#tryEvaluateExpression(rawASTExpr, node.parent ?? ({} as ModelicaClassInstance));
        if (evalResult) {
          arrayBindingExpression = evalResult;
          // Successfully evaluated to a literal/array; skip syntax flattener fallback below
        }
      }
    }

    // Re-evaluate condition in case tryEvaluateExpression succeeded
    const stillNeedsSyntaxFlattener =
      !arrayBindingExpression ||
      (arrayBindingExpression instanceof ModelicaArray && arrayBindingExpression.flatShape.includes(0)) ||
      !isCompileTimeEvaluableEarly;

    if (stillNeedsSyntaxFlattener && rawASTExpr) {
      ...
      if (syntaxResult) {
        if (
          !isCompileTimeEvaluableEarly &&
          savedBinding &&
          !(savedBinding instanceof ModelicaArray && savedBinding.flatShape.includes(0))
        ) {
          ...
          try {
            ...
            if (exprNode.kind === "expression" && exprNode.text) {
              ...
              try {
                ...
                extracted = findExpr(root);
              } catch {
                /* ignore */
              }
              if (extracted) {
                exprNode = ModelicaSyntaxNode.new(null, extracted);
              }
            }
            const interp = new ModelicaInterpreter(true);
            ...
            const classInst = (val as any)?.classInstance;
            if (classInst && classInst.constructor.name === "ModelicaEnumerationClassInstance") {
              const lits = classInst.enumerationLiterals;
              if (lits && Array.isArray(lits)) {
                let fqn = classInst.name;
                let curr = classInst.parent;
                while (curr && curr.name) {
                  fqn = curr.name + "." + fqn;
                  curr = curr.parent;
                }
                enumDimensions.set(i, {
                  typeName: fqn,
                  literals: lits,
                });
    ...
    if (shape.length >= 1 && shape.some((d: any) => d === 0) && arrayBindingExpression instanceof ModelicaArray) {
      shape = arrayBindingExpression.flatShape;
    ...
    const predefinedBase_first = declaredElements.length > 0 ? getUnderlyingPredefinedClass(declaredElements[0]) : null;
    const isPrimitiveType =
      predefinedBase_first ||
      (declaredElements.length > 0 && declaredElements[0] instanceof ModelicaEnumerationClassInstance);

    // Pre-evaluate attributes for primitive elements outside the loop
    const preEvaluatedAttributes = new Map<string, any>();
    if (isPrimitiveType && declaredElements.length > 0) {
      const firstElement = declaredElements[0];
      const baseAttributes = new Map<string, ModelicaExpression>();
      this.#collectTypeAttributes(firstElement, baseAttributes);
      for (const m of firstElement.modification?.modificationArguments ?? []) {
        if (m.name && m.expression) baseAttributes.set(m.name, m.expression);
      }
      for (const m of node.modification?.modificationArguments ?? []) {
        if (m.name && m.name !== "annotation" && m.expression) baseAttributes.set(m.name, m.expression);
      }
      for (const pm of currentMods) {
        if (pm.nameParts.length === 1 && pm.nameParts[0] !== "annotation") {
          baseAttributes.set(pm.nameParts[0] ?? "", pm.valueExpr);
        }
      }
      for (const [key, val] of baseAttributes.entries()) {
        preEvaluatedAttributes.set(key, this.#flattenToSymbolic(val, node, args[0], args[1]));
      }
    }

    // Pre-evaluate currentMods for compound elements outside the loop
    const currentModsFlatVals = new Map<any, any>();
    if (!isPrimitiveType) {
      for (const mod of currentMods) {
        currentModsFlatVals.set(mod, this.#flattenToSymbolic(mod.valueExpr, node, args[0], args[1]));
      }
    }

    // Pre-evaluate component's own modifications for compound elements outside the loop
    const compModsFlatVals = new Map<any, any>();
    if (!isPrimitiveType && node.modification) {
      for (const modArg of node.modification.modificationArguments ?? []) {
        const isRedeclare = modArg.ast?.redeclare || modArg.arg?.redeclare || modArg.isRedeclare;
        ...
        for (const subMod of subModArgs) {
          const subExpr = subMod.modificationExpression?.expression || subMod.expression || subMod.arg?.value;
          if (subExpr) {
            compModsFlatVals.set(subMod, this.#flattenToSymbolic(subExpr, node, args[0], args[1]));
          }
        }
      }
    }
    ...
    for (const declaredElement of declaredElements) {
      ...
      const predefinedBase = getUnderlyingPredefinedClass(declaredElement);

      if (predefinedBase || declaredElement instanceof ModelicaEnumerationClassInstance) {
        const attributes = new Map<string, ModelicaExpression>();

        // Apply pre-evaluated symbolic expressions
        for (const [key, preEvalVal] of preEvaluatedAttributes.entries()) {
          let evalVal = preEvalVal;
          if (evalVal instanceof ModelicaArray) {
            const subscripts = index.map((idx: number) => AstBuilder.int(idx));

            // The attribute might be defined on a base type (e.g. T = Real[3,2]) and thus only have shape [3,2],
            // while the component has shape [4,1,3,2]. We take the trailing subscripts matching the array's rank.
            const rank = evalVal.flatShape.length;
            const relevantSubscripts = rank > 0 && rank <= subscripts.length ? subscripts.slice(-rank) : subscripts;

            let extracted = true;
            let currentVal: ModelicaExpression | null = null;

            // Calculate flat index into the array
            let flatIdx = 0;
            let stride = 1;
            const shape = evalVal.flatShape;
            for (let dim = relevantSubscripts.length - 1; dim >= 0; dim--) {
              const sub = relevantSubscripts[dim];
              const idx = sub?.value;
              if (idx !== undefined) {
                flatIdx += (idx - 1) * stride;
                stride *= shape[dim] ?? 1;
              } else {
                extracted = false;
                break;
            ...
            if (extracted) {
              // Ensure we extract from the flattened elements array
              const flatElements = [...evalVal.flatElements];
              if (flatIdx >= 0 && flatIdx < flatElements.length) {
                currentVal = flatElements[flatIdx] as ModelicaExpression;
              } else {
                extracted = false;
              }
            }
            if (extracted && currentVal) {
              evalVal = currentVal;
            } else {
              evalVal = AstBuilder.subscript(evalVal, relevantSubscripts);
            }
          }
        ...
        if (flatBindingElements) {
          ...
          const subscripts = index.map((idx: number) => AstBuilder.int(idx));
          expression = AstBuilder.subscript(arrayBindingExpression, subscripts);
        ...
        const targetBase = predefinedBase ?? declaredElement;

        if (targetBase.name === "Boolean") {
          variable = new ModelicaBooleanVariable(
            elementName,
            varExpression,
            attributes as any,
        ...
        } else if (targetBase.name === "Integer") {
          variable = new ModelicaIntegerVariable(
            elementName,
            varExpression,
            attributes as any,
        ...
        } else if (targetBase.name === "Real") {
          for (const key of ["start", "min", "max", "nominal"]) {
            if (attributes.has(key)) {
              const casted = castToReal((attributes.get(key) as any) ?? null);
              if (casted) attributes.set(key, casted);
            }
          }
          variable = new ModelicaRealVariable(
        ...
        } else if (targetBase.name === "String") {
          variable = new ModelicaStringVariable(
            elementName,
            varExpression,
            attributes as any,
        ...
        // For compound types (records, models), propagate outer variability,
        // final, and protected — mirrors visitComponentInstance behavior.
        const savedVar = this.#outerVariability;
        const savedFinal = this.#outerFinal;
        const savedProtected = this.#outerProtected;
        this.#outerVariability = node.variability ?? this.#outerVariability;
        this.#outerFinal = this.#outerFinal || node.isFinal;
        ...
        for (const mod of currentMods) {
          ...
          const flatVal = currentModsFlatVals.get(mod);
              ...
              if (sliceElements.length > 0) {
                slicedExpr =
                  sliceElements.length === 1
                    ? sliceElements[0]
                    : AstBuilder.array(flatVal.flatShape.slice(1), sliceElements);
              }
            } else {
        ...
        if (compMod) {
          for (const modArg of compMod.modificationArguments ?? []) {
            ...
            for (const subMod of subModArgs) {
              ...
              const flatVal = compModsFlatVals.get(subMod);
    ...
    if (arrayBindingExpression && !flatBindingElements && !isCompileTimeEvaluable && (shape[0] ?? 0) > 0) {
      ...
      const elementType = arrayClassInstance.elementClassInstance ?? Array.from(arrayClassInstance.declaredElements)[0];
      const isRealArray =
      ...
      if (rhs instanceof ModelicaNameExpression || rhs instanceof ModelicaVariable) {
        ...
        if (totalElements > 0) {
          ...
          for (let i = 1; i <= totalElements; i++) {
            elements.push(AstBuilder.subscript(rhs, [AstBuilder.int(i)]));
          }
          rhs = AstBuilder.array([totalElements], elements);
        }
      }
   ...
   * @param args - A tuple of `[prefixString, activeDAE]`.
   */
  visitExtendsClassInstance(node: any /* ModelicaExtendsClassInstance */, args: [string, ModelicaDAE]): void {
  ...
  foldDAEConstants(dae: ModelicaDAE) {
    ...
    while (changed && iterations < 100) {
      changed = false;
      iterations++;
      for (const variable of dae.arenaVariables()) {
        if (
          ...
          if (variable.expression) {
            ...
            if (variable instanceof ModelicaIntegerVariable && newExpr instanceof ModelicaRealLiteral) {
              ...
              if (newExpr.value >= 8e304) newExpr = AstBuilder.int(0, "4611686018427387903");
              else if (newExpr.value <= -8e304) newExpr = AstBuilder.int(0, "-4611686018427387903");
              else newExpr = AstBuilder.int(Math.trunc(newExpr.value));
              ...
              if (newExpr.value > 0) newExpr = AstBuilder.bool(true);
              else newExpr = AstBuilder.bool(false);
            } else if (variable instanceof ModelicaBooleanVariable && newExpr instanceof ModelicaIntegerLiteral) {
              newExpr = AstBuilder.bool(newExpr.value !== 0);
            }
            ...
            if (newExpr1 instanceof ModelicaNameExpression && newExpr2 instanceof ModelicaArray) {
              ...
              if (vars.length === flat2.length && vars.length > 0) {
                for (let i = 0; i < vars.length; i++) {
                  ...
                  const rhs = flat2[i];
                  if (!v || !rhs) continue;
                  const lhs = AstBuilder.name(v.name);
              ...
              if (vars.length === flat1.length && vars.length > 0) {
                for (let i = 0; i < vars.length; i++) {
                  ...
                  const lhs = flat1[i];
                  if (!v || !lhs) continue;
                  const rhs = AstBuilder.name(v.name);
                  newEquations.push(new ModelicaSimpleEquation(lhs, rhs, equation.description));
                }
                expanded = true;
                changed = true;
  ...
  #foldExpression(
    ...
    if (expr instanceof ModelicaBinaryExpression) {
      ...
      if (isLiteral(op1)) {
        if (expr.operator === ModelicaUnaryOperator.UNARY_MINUS) {
          if (op1 instanceof ModelicaRealLiteral) return AstBuilder.real(-op1.value);
          if (op1 instanceof ModelicaIntegerLiteral) return AstBuilder.int(-op1.value);
        } else if (expr.operator === ModelicaUnaryOperator.UNARY_PLUS) {
          return op1;
        } else if (expr.operator === ModelicaUnaryOperator.LOGICAL_NEGATION) {
          if (op1 instanceof ModelicaBooleanLiteral) return AstBuilder.bool(!op1.value);
        }
      }
      ...
      if (expr.operator === ModelicaUnaryOperator.UNARY_MINUS && op1 instanceof ModelicaArray) {
        const foldedElements = op1.elements.map((e) =>
          this.#foldExpression(AstBuilder.unary(ModelicaUnaryOperator.UNARY_MINUS, e), dae, visited, inlineParameters),
        );
        return AstBuilder.array(op1.shape, foldedElements);
      }
      return op1 === expr.operand ? expr : AstBuilder.unary(expr.operator, op1);
      ...
      if (args.length === 1 && args[0] instanceof ModelicaArray) {
        ...
        if (!nonDistributive.has(expr.functionName) && !isUserDefined) {
          const newElements = arr.elements.map((e) =>
            this.#foldExpression(AstBuilder.call(expr.functionName, [e]), dae, visited, inlineParameters),
          );
          return AstBuilder.array(arr.shape, newElements);
        }
      }
      return AstBuilder.call(expr.functionName, args);
      ...
      // Try to expand the comprehension if the range is resolvable
      if (dae && newIterators.length === 1 && newIterators[0]) {
        const iter = newIterators[0];

        let elementsArray: ModelicaExpression[] | null = null;
        if (iter.range instanceof ModelicaArray) {
          elementsArray = iter.range.elements;
        } else {
          const rangeValues = evaluateRangeExpr(iter.range);
          if (rangeValues) {
            elementsArray = rangeValues.map((v) => AstBuilder.int(v));
          }
        }

        if (elementsArray) {
          const elements: ModelicaExpression[] = [];
          for (const val of elementsArray) {
            const substituted = substituteIterator(newBody, iter.name, val);
            elements.push(this.#foldExpression(substituted, dae, visited, inlineParameters));
          }

          if (expr.functionName === "sum") {
            if (elements.length === 0) return AstBuilder.real(0.0);
            return elements.reduce((acc, curr) => AstBuilder.binary(ModelicaBinaryOperator.ADDITION, acc, curr));
          } else if (expr.functionName === "product") {
            if (elements.length === 0) return AstBuilder.real(1.0);
            return elements.reduce((acc, curr) => AstBuilder.binary(ModelicaBinaryOperator.MULTIPLICATION, acc, curr));
          } else if (expr.functionName === "min") {
            if (elements.length === 0) return AstBuilder.array([0], []);
            return elements.reduce((acc, curr) => AstBuilder.call("min", [acc, curr]));
          } else if (expr.functionName === "max") {
            if (elements.length === 0) return AstBuilder.array([0], []);
            return elements.reduce((acc, curr) => AstBuilder.call("max", [acc, curr]));
          }

          return AstBuilder.array([elements.length], elements);
        }

        // Record array range: {expr for i in r} where r is a known array in the DAE
        if (iter.range instanceof ModelicaNameExpression || iter.range instanceof ModelicaVariable) {
          const arrayName = iter.range instanceof ModelicaVariable ? iter.range.name : iter.range.name;
          const arrayVars = dae.arenaGetArrayElements(arrayName);
          if (arrayVars.length > 0) {
            // Extract unique first-dimension indices
            const indices = new Set<number>();
            for (const v of arrayVars) {
              const sub = v.name.substring(arrayName.length + 1); // after "["
            ...
            if (indices.size > 0) {
              ...
              for (const idx of sortedIndices) {
                // Substitute iterator with subscripted array element: i → r[k]
                const elemName = `${arrayName}[${idx}]`;
                const substituted = substituteIterator(newBody, iter.name, AstBuilder.name(elemName));
                elements.push(this.#foldExpression(substituted, dae, visited, inlineParameters));
              }
              return AstBuilder.array([elements.length], elements);
            }
    ...
    } else if (expr instanceof ModelicaRangeExpression) {
      const start = this.#foldExpression(expr.start, dae, visited, inlineParameters);
      const end = this.#foldExpression(expr.end, dae, visited, inlineParameters);
      const step = expr.step ? this.#foldExpression(expr.step, dae, visited, inlineParameters) : null;
      return new ModelicaRangeExpression(start, end, step);
    } else if (expr instanceof ModelicaEnumerationLiteral) {
      return expr;
      ...
      return AstBuilder.array(expr.shape, newElements);
      ...
      if (dae && !visited.has(expr.name)) {
        const variable = dae.arenaGetVarByName(expr.name);
        if (
          variable &&
          (variable.variability === ModelicaVariability.CONSTANT ||
      ...
      if (dae) {
        ...
        if (base instanceof ModelicaNameExpression && subscripts.every((s) => s instanceof ModelicaIntegerLiteral)) {
          ...
          if (!visited.has(flatName)) {
            const variable = dae.arenaGetVarByName(flatName);
            if (
              variable &&
              (variable.variability === ModelicaVariability.CONSTANT ||
        ...
        if (base instanceof ModelicaNameExpression && isSlice) {
          ...
          const resolvedSubscripts = subscripts.map((s, d) => {
            if (s instanceof ModelicaColonExpression) {
              const sizeExpr = AstBuilder.call("size", [base, AstBuilder.int(d + 1)]);
              const sizeVal = this.#foldExpression(sizeExpr, dae, visited, inlineParameters);
              return new ModelicaRangeExpression(AstBuilder.int(1), sizeVal);
            }
            return s;
          });
          ...
          for (const s of resolvedSubscripts) {
            if (s instanceof ModelicaRangeExpression) {
              ...
              if (startVal instanceof ModelicaIntegerLiteral && endVal instanceof ModelicaIntegerLiteral) {
                ...
                return AstBuilder.subscript(base, subscripts); // fallback if variable size
              }
            } else if (s instanceof ModelicaArray) {
              const arr = [];
              for (const el of s.flatElements) {
                if (!el) return AstBuilder.subscript(base, subscripts);
                ...
                if (foldedEl instanceof ModelicaIntegerLiteral) {
                  ...
                  return AstBuilder.subscript(base, subscripts); // fallback for symbolic subscripts
                }
              }
              axes.push(arr);
            } else {
          ...
          const buildNestedArray = (axisIndex: number, currentIndices: number[]): ModelicaExpression => {
            if (axisIndex >= axes.length) {
              ...
              return AstBuilder.subscript(
                base,
                currentIndices.map((i) => AstBuilder.int(i)),
              );
            }
        ...
        if (
          ...
          if (comp.iterators.length === 1 && comp.iterators[0]) {
            ...
            const substituted = substituteIterator(comp.bodyExpression, iterName, AstBuilder.int(iterValue));
        ...
        if (
          ...
          const flatName = base.name + "[" + subscripts.map((s) => (s as ModelicaIntegerLiteral).value).join(",") + "]";
          if (dae.arenaHasVar(flatName)) {
            return AstBuilder.name(flatName);
          }
        }
  ...
  #assembleStateMachines(dae: ModelicaDAE): void {
    ...
    for (const comp of components) {
      ...
      for (const fullName of comp) {
        ...
        if (contents) {
          ...
          stateNode.variables = [...dae.arenaVariables()].filter((v: ModelicaVariable) => varSet.has(v));
          for (const v of varSet) {
            if (v._arenaIdx !== undefined) dae.arena.setVarRemoved(v._arenaIdx);
          }
    ...
    const sortedMachines = Array.from(allMachines.values()).sort((a: any, b: any) => b.rank - a.rank);
  ...
  #partitionClocks(dae: ModelicaDAE): void {
    ...
    for (const call of clockExprs.values()) {
      ...
      const clockArg = call.args.find(
        (a: any) => a instanceof ModelicaFunctionCallExpression && a.functionName === "Clock",
      );
    ...
    const globalBaseClock = hasFractionalClocks
      ? AstBuilder.call("Clock", [AstBuilder.int(baseNum), AstBuilder.int(baseDen)])
      : null;
    ...
    const extractExpr = (expr: ModelicaExpression) => {
      if (expr instanceof ModelicaBinaryExpression) {
        if (
          ...
          const diff = AstBuilder.binary(ModelicaBinaryOperator.SUBTRACTION, expr.operand1, expr.operand2);
...
class ModelicaSyntaxFlattener extends ModelicaSyntaxVisitor<ModelicaExpression, FlattenerContext> {
  public options: Required<Pick<ModelicaCompilerOptions, "arrayMode" | "functionInlining">> & ModelicaCompilerOptions;
  ...
  visitArrayConcatenation(
    ...
    if (rows.length === 0) return AstBuilder.array([0], []);

    // Try to evaluate statically if all elements are literals/arrays
    const isStatic = rows.every((row) =>
      row.every((e) => e instanceof ModelicaArray || e.constructor.name.endsWith("Literal")),
    );
    ...
    if (isStatic) {
      ...
      if (allScalars) {
        if (rows.length === 1) {
          const row0 = rows[0] || [];
          return AstBuilder.array([row0.length], row0);
        } else {
          const flatElements: ModelicaExpression[] = [];
          for (const row of rows) flatElements.push(...row);
          return AstBuilder.array([rows.length, rows[0]?.length ?? 0], flatElements);
        }
      } else {
        const flatElements: ModelicaExpression[] = [];
        let numRows: number;
        ...
        if (rows.length === 1) {
          ...
          if (first instanceof ModelicaArray && first.shape.length === 1) {
            ...
            return AstBuilder.array([numRows, numCols], flatElements);
          } else {
    ...
    // Fallback to cat() translations for symbolic expressions
    const listExprs: ModelicaExpression[] = [];
    for (const rowExprs of rows) {
      if (rowExprs.length === 1) {
        const rowExpr = rowExprs[0];
        if (rowExpr) {
          // Wrap in cat(2, ...) even for single elements if it's a multi-row matrix [e1; e2]
          if (rows.length > 1) {
            listExprs.push(AstBuilder.call("cat", [AstBuilder.int(2), rowExpr]));
          } else {
            listExprs.push(rowExpr);
          }
        }
      } else if (rowExprs.length > 1) {
        listExprs.push(AstBuilder.call("cat", [AstBuilder.int(2), ...rowExprs]));
      }
    }
    ...
    if (listExprs.length === 1) {
      if (rows.length === 1 && rows[0]?.length === 1) {
        ...
        if (expr) {
          if (expr instanceof ModelicaArray) {
            finalExpr = AstBuilder.call("cat", [AstBuilder.int(1), expr]);
          } else {
            finalExpr = AstBuilder.array([1, 1], [expr]);
          }
        }
      } else {
      ...
      finalExpr = AstBuilder.call("cat", [AstBuilder.int(1), ...listExprs]);
    }

    if (!finalExpr) return AstBuilder.array([0], []);
  ...
  visitArrayConstructor(node: ModelicaArrayConstructorSyntaxNode, ctx: FlattenerContext): ModelicaExpression | null {
    ...
    if (node.comprehensionClause?.expression && node.comprehensionClause.forIndexes.length > 0) {
      ...
      const iterate = (depth: number, innerCtx: FlattenerContext): void => {
        ...
        // Handle record/component array iteration: {expr for i in r}
        // If rangeExpr was evaluated to a ModelicaArray, process its elements directly.
        if (rangeExpr && rangeExpr.constructor.name.includes("ModelicaArray")) {
          let hasComponentVars = false;
          const uniquePrefixes = new Set<string>();
          const flatElements = (rangeExpr as any).flatElements;
          for (const el of flatElements) {
            if (el && typeof el === "object" && "name" in el && typeof el.name === "string" && el.name.includes("[")) {
              hasComponentVars = true;
              const bracketEnd = el.name.indexOf("]");
              if (bracketEnd > 0) {
                uniquePrefixes.add(el.name.substring(0, bracketEnd + 1)); // e.g., "r[1]"
              }
            }
          }
          if (hasComponentVars && uniquePrefixes.size > 0) {
            const sortedPrefixes = Array.from(uniquePrefixes).sort((a, b) => {
              const aIdx = parseInt(a.substring(a.indexOf("[") + 1, a.indexOf("]")));
              const bIdx = parseInt(b.substring(b.indexOf("[") + 1, b.indexOf("]")));
              return aIdx - bIdx;
            });
            for (const prefix of sortedPrefixes) {
              const loopVars = new Map(innerCtx.loopVariables ?? []);
              loopVars.set(indexName, AstBuilder.name(prefix));
              iterate(depth + 1, { ...innerCtx, loopVariables: loopVars });
            }
            return;
          } else if (!hasComponentVars) {
            // Treat as array of literals
            for (const el of flatElements) {
              const loopVars = new Map(innerCtx.loopVariables ?? []);
              loopVars.set(indexName, el);
              iterate(depth + 1, { ...innerCtx, loopVariables: loopVars });
            }
            return;
          }
        ...
        // Handle standard numeric ranges like 1:3
        ...
        for (const val of values) {
          const loopVars = new Map(innerCtx.loopVariables ?? []);
          loopVars.set(indexName, AstBuilder.int(val));
          iterate(depth + 1, { ...innerCtx, loopVariables: loopVars });
        }
      };
      ...
      if (rangeResolvable) {
        if (results.length === 0) return AstBuilder.array([0], []);
        return AstBuilder.array([results.length], results);
      }
      ...
      for (const forIndex of comp.forIndexes) {
        ...
        if (iterName && range) {
          ...
          loopVars.set(iterName, AstBuilder.name(iterName));
        }
      }
    ...
    if (elements.length > 0 && elements.every((e) => e instanceof ModelicaArray)) {
      ...
      if (allSameShape) {
        ...
        return AstBuilder.array([elements.length, ...innerShape], flatElements);
      }
    }
    return AstBuilder.array([elements.length], elements);
  }
  ...
  visitBinaryExpression(node: ModelicaBinaryExpressionSyntaxNode, ctx: FlattenerContext): ModelicaExpression | null {
    ...
    // rewrite `a op b` as `RecordType.'op'.funcName(a: any, b: any)` or `RecordType.'op'(a: any, b: any)`.
    ...
    if (operatorInfo) {
      ...
      return AstBuilder.call(qualifiedName, [operand1, operand2]);
    }
  ...
  #resolveOperatorRecordFunction(
    ...
    operatorName: string,
    ctx: FlattenerContext,
  ): { qualifiedName: string; resolvedClass: any } | null {
    ...
    const resolved: any = ctx.classInstance.resolveName(componentNames);
    if (!resolved?.isComponentInstance) return null;
    ...
    for (const el of declaredType.elements) {
      if (!el.isClassInstance) continue;
      ...
      if (el.classKind === ModelicaClassKind.OPERATOR && el.name === operatorName) {
        if (el.instantiate) el.instantiate();
        // Find the first function inside the operator
        for (const fn of el.elements) {
          if (fn.isClassInstance && fn.classKind === ModelicaClassKind.FUNCTION && fn.name) {
  ...
  visitBooleanLiteral(node: ModelicaBooleanLiteralSyntaxNode): ModelicaBooleanLiteral {
    return AstBuilder.bool(node.value);
  }
  ...
  visitFunctionCall(node: ModelicaFunctionCallSyntaxNode, ctx: FlattenerContext): ModelicaExpression | null {
    ...
    if (boundArgsMap && boundArgsMap.size > 0) {
      ...
      if (
        ...
        const inputs = Array.from(resolved.components).filter((c: any) => c.causality?.toString() === "input");
      ...
      if (
        ...
        for (const comp of inputComponents) {
          ...
          if (namedArgsMap.has(compName)) {
            orderedArgs.push(namedArgsMap.get(compName) ?? AstBuilder.int(0));
        ...
        for (const comp of protectedComponents) {
          ...
          if (rawExpr != null) {
            ...
            if (typeof rawExpr === "number") {
              defaultVal = AstBuilder.real(rawExpr);
            } else if (typeof rawExpr === "string") {
              defaultVal = AstBuilder.string(rawExpr);
            } else if (typeof rawExpr === "boolean") {
              defaultVal = AstBuilder.bool(rawExpr);
        ...
        return AstBuilder.call(qualifiedName, flatArgs);
      }
    }
    ...
    if (functionName === "zeros" && flatArgs.length >= 1) {
      ...
      if (arg0) {
        ...
        if (zeroOp) {
          ...
          this.#collectFunctionDefinition(qualifiedName, ctx, resolvedClass);
          return AstBuilder.call(qualifiedName, []);
        }
      }
    }
      ...
      if (bodyExpr) {
        ...
        if (
          ...
          if (constants.length > 0 && symbolics.length === 0 && builtinDef.foldConstants) {
            ...
            const foldedConst = builtinDef.foldConstants(constants);
            return Number.isInteger(foldedConst) ? AstBuilder.int(foldedConst) : AstBuilder.real(foldedConst);
          }
        }
    ...
    if (functionName === "getInstanceName") {
      ...
      return AstBuilder.string(instanceName);
    }
      ...
      if (v1 instanceof ModelicaArray && v2 instanceof ModelicaArray) {
        ...
        for (let i = 0; i < n && allConstant; i++) {
          ...
          for (let j = 0; j < m; j++) {
            ...
            if (a != null && b != null) {
              rowElements.push(AstBuilder.real(a * b));
            } else {
              allConstant = false;
              break;
            }
          }
          if (allConstant) rows.push(AstBuilder.array([m], rowElements));
        }
        if (allConstant) return AstBuilder.array([n], rows);
      }
    ...
    if (builtinDef) {
      while (flatArgs.length < builtinDef.inputs.length) {
        ...
        if (typeof param.defaultValue === "boolean") {
          flatArgs.push(AstBuilder.bool(param.defaultValue));
        } else {
          flatArgs.push(AstBuilder.int(param.defaultValue));
        }
      }
    }
    ...
    if (functionName === "linspace" && flatArgs.length >= 3) {
      ...
      if (n && !(n instanceof ModelicaIntegerLiteral)) {
        ...
        const iMinus1 = AstBuilder.binary(ModelicaBinaryOperator.ADDITION, AstBuilder.int(-1), AstBuilder.name("i"));
        const nMinus1 = AstBuilder.binary(ModelicaBinaryOperator.ADDITION, AstBuilder.int(-1), n);
        const x1Val =
        ...
        if (x1Val === 0 && x2Val === 1) {
          ...
          body = AstBuilder.binary(
            ModelicaBinaryOperator.DIVISION,
            AstBuilder.call("/*Real*/", [iMinus1]),
            AstBuilder.call("/*Real*/", [nMinus1]),
          );
        } else {
          // General case: x1 + (x2-x1) * /*Real*/(-1+i) / /*Real*/(-1+n)
          const range =
            x2Val != null && x1Val != null
              ? AstBuilder.real(x2Val - x1Val)
              : AstBuilder.binary(
                  ModelicaBinaryOperator.SUBTRACTION,
                  x2 ?? AstBuilder.real(0),
                  x1 ?? AstBuilder.real(0),
                );
          const ratio = AstBuilder.binary(
            ModelicaBinaryOperator.DIVISION,
            AstBuilder.call("/*Real*/", [iMinus1]),
            AstBuilder.call("/*Real*/", [nMinus1]),
          );
          const scaled = AstBuilder.binary(ModelicaBinaryOperator.MULTIPLICATION, range, ratio);
          body =
            x1Val === 0 ? scaled : AstBuilder.binary(ModelicaBinaryOperator.ADDITION, x1 ?? AstBuilder.real(0), scaled);
        }
        return new ModelicaComprehensionExpression("array", body, [
          { name: "i", range: new ModelicaRangeExpression(AstBuilder.int(1), n, null) },
        ]);
      }
    }
    ...
    if (builtinDef && builtinDef.inputs.length === 1 && builtinDef.fold1 && flatArgs.length === 1) {
      const arg = flatArgs[0];
      if (arg instanceof ModelicaNameExpression) {
        const isArrayVar = !!ctx.dae.arenaGetEncoded(arg.name) || ctx.dae.arenaHasArrayElements(arg.name);
        // For function body context: check if the arg is a known array-typed param
        const funcDef = ctx.dae.functions.find((f) =>
          f.legacyVariables.some(
        ...
        if (isArrayVar || funcDef) {
          ...
          const bodyExpr = AstBuilder.call(functionName, [AstBuilder.name(tmpVarName)]);
    ...
    if (builtinDef) {
      ...
      if (builtinDef.overloads && builtinDef.overloads.length > 0) {
        ...
        const argIsInteger = (arg: ModelicaExpression): boolean => {
          ...
          if (arg instanceof ModelicaNameExpression) {
            const foundVar = ctx.dae.arenaGetVarByName(arg.name);
    ...
    if (functionName === "sample" && flatArgs.length === 1) {
      ...
      if (flatArgs[0] && isRealTyped(flatArgs[0], ctx.dae)) {
        flatArgs.push(AstBuilder.call("Clock", []));
      }
    }
    ...
    if (!builtinDef) {
      ...
      if (funcDef) {
        const inputVars = funcDef.legacyVariables.filter((v) => v.causality === "input");
        const funcLocalNames = new Set(funcDef.legacyVariables.map((v) => v.name));
      ...
      if (funcDef) {
        ...
        if (vectorizationSize > 0 && allScalarParamsHaveArrayArgs) {
          ...
          if (rejectedVectorization) {
            ...
            const outputVars = funcDef.legacyVariables.filter((v) => v.causality === "output");
        ...
        if (vectorizationSize > 0 && allScalarParamsHaveArrayArgs) {
          ...
          for (let ei = 0; ei < vectorizationSize; ei++) {
            ...
            scalarCalls.push(AstBuilder.call(callName, scalarArgs));
          }
            ...
            for (let ci = 0; ci < scalarCalls.length; ci++) {
              ...
              try {
                ...
                for (let pi = 0; pi < call.args.length && pi < funcInputParams.length; pi++) {
                  const pName = (funcInputParams[pi] as any)?.name;
                  if (pName && call.args[pi]) {
                    params.push(
                      new ModelicaParameterModification(
                        ctx.classInstance as any,
                        pName,
                        null,
                ...
                const mod = new (ModelicaModification as any)(ctx.classInstance as any, params);
          ...
          return AstBuilder.array(vectorizationShape, scalarCalls);
        }
        ...
        if (vectorizationSize <= 0) {
          ...
          for (let i = 0; i < flatArgs.length && i < inputVars.length; i++) {
            if (!isScalarParam[i] && flatArgs[i] instanceof ModelicaArray) {
              ...
              if (argDimCount > declaredDimCount) {
                ...
                const vecSize = extraDims.reduce((a: any, b: any) => a * b, 1);
                if (arrayVecSize < 0) {
          ...
          if (arrayVecSize > 0) {
              ...
              for (let ai = 0; ai < flatArgs.length && ai < inputVars.length; ai++) {
                if (isScalarParam[ai]) {
                  ...
                  if (argDimCount > declaredDimCount) {
                    ...
                    const innerSize = arr.shape
                      .slice(argDimCount - declaredDimCount)
                      .reduce((a: any, b: any) => a * b, 1);
                    ...
                    if (innerShape.length === 0) {
                      ...
                      scalarArgs.push(AstBuilder.array(innerShape, sliceElements));
                    }
                  } else {
                    scalarArgs.push(arr);
                  }
            ...
            return AstBuilder.array([arrayVecSize], scalarCalls);
          }
        }
    ...
    if (!builtinDef) {
      ...
      if (fDef) {
        const inputVars = fDef.legacyVariables.filter((v) => v.causality === "input");
        ...
        if (dotIdx >= 0) {
          ...
          if (resolvedConnector && resolvedConnector.isComponentInstance) {
            ...
            if (flowVars.size > 0) {
              ...
              let flowExpr: ModelicaExpression = AstBuilder.name(firstFlowVar);
              for (let i = 1; i < flowVarsArr.length; i++) {
                const ithFlowVar = flowVarsArr[i] as string;
                flowExpr = AstBuilder.binary(ModelicaBinaryOperator.ADDITION, flowExpr, AstBuilder.name(ithFlowVar));
              }
              return new ModelicaIfElseExpression(
                AstBuilder.binary(ModelicaBinaryOperator.GREATER_THAN, flowExpr, AstBuilder.real(0)),
                AstBuilder.name(`$inStream(${vName})`),
                [],
    ...
    const result = AstBuilder.call(finalFunctionName, finalArgs);

    // Only inline user-defined function calls when ALL arguments are compile-time constants.
    // By default, parameters are NOT constants — they can change between simulations.
    // However, if options.functionInlining === "inline" (e.g. in tests), we treat parameters as constants.
    ...
    const isConstantEvaluable = (expr: ModelicaExpression): boolean => {
      ...
      if (expr instanceof ModelicaNameExpression) {
        const variable = ctx.dae.arenaGetVarByName(expr.name);
        if (variable) {
          const treatAsConstant =
            variable.variability === ModelicaVariability.CONSTANT ||
            (this.options?.functionInlining === "inline" && variable.variability === ModelicaVariability.PARAMETER);
          if (treatAsConstant && variable.expression) {
          ...
          if (resolved instanceof ModelicaComponentInstance) {
            const treatAsConstant =
              ...
              (this.options?.functionInlining === "inline" && resolved.variability === ModelicaVariability.PARAMETER);
            if (treatAsConstant && resolved.modification?.expression) {
      ...
      if (expr instanceof ModelicaVariable) {
        const treatAsConstant =
          expr.variability === ModelicaVariability.CONSTANT ||
          (this.options?.functionInlining === "inline" && expr.variability === ModelicaVariability.PARAMETER);
        if (treatAsConstant && expr.expression) {
        ...
        if (!builtinDef && flatArgs.every((a) => isLiteral(a) || isLiteralArray(a))) {
          ...
          if (funcInstance instanceof ModelicaClassInstance && funcInstance.classKind === ModelicaClassKind.FUNCTION) {
            ...
            if (flatArgs.length <= inputParams.length) {
              const params: ModelicaParameterModification[] = [];
              for (let i = 0; i < flatArgs.length; i++) {
                const pName = (inputParams[i] as any)?.name;
                if (pName && flatArgs[i]) {
                  params.push(new ModelicaParameterModification(ctx.classInstance as any, pName, null, flatArgs[i]));
                }
              }
              const mod = new (ModelicaModification as any)(ctx.classInstance as any, params);
              if (funcInstance.abstractSyntaxNode) {
  ...
  #resolveFullyQualifiedName(functionName: string, ctx: FlattenerContext): string {
    const parts = functionName.split(".");
    const resolved: any = ctx.classInstance.resolveName(parts);
    if (!resolved || !(resolved.isClassInstance || resolved.isComponentInstance)) return functionName;
  ...
  #resolveComponentScopedFunction(
    ...
    if (!(resolved instanceof ModelicaClassInstance)) return null;
    if (
      (resolved as any).classKind !== ModelicaClassKind.FUNCTION &&
      (resolved as any).classKind !== ModelicaClassKind.OPERATOR_FUNCTION
    ) {
      return null;
    }
  ...
  #collectRecordConstructor(recordName: string, recordClass: ModelicaClassInstance, ctx: FlattenerContext): void {
    ...
    for (const comp of recordClass.components) {
      ...
      if (isFinalOrConstant) {
        ...
        if (rawExpr != null) {
          if (typeof rawExpr === "number") {
            defaultExpr = Number.isInteger(rawExpr) ? AstBuilder.real(rawExpr) : AstBuilder.real(rawExpr);
          } else if (typeof rawExpr === "string") {
            defaultExpr = AstBuilder.string(rawExpr);
          } else if (typeof rawExpr === "boolean") {
            defaultExpr = AstBuilder.bool(rawExpr);
          } else if (rawExpr instanceof ModelicaExpression) {
            defaultExpr = rawExpr;
          } else if (typeof rawExpr?.accept === "function") {
            // AST node — flatten it
      ...
      fnDae.legacyVariables.push(variable);
    }
    ...
    fnDae.legacyVariables.push(outputVar);
  ...
  #collectFunctionDefinition(
    ...
    fnDae.description =
      resolved.abstractSyntaxNode?.classSpecifier?.description?.strings?.map((d: any) => d.text ?? "")?.join(" ") ??
      null;
    ...
    for (const algorithmSection of allAlgorithmSections) {
      ...
      if (collector.length > 0) {
        ...
        const markRecursiveReturns = (stmts: ModelicaStatement[]): void => {
          ...
          const hasOutput = fnDae.legacyVariables.some((v) => v.causality === "output");
          if (!hasOutput) return;
    ...
    if (classSpecifier instanceof ModelicaLongClassSpecifierSyntaxNode) {
      ...
      if (ext) {
        ...
        if (call) {
          ...
          for (const v of fnDae.legacyVariables) {
  ...
  visitComponentReference(
    ...
    if (typeof simpleNameStr === "string" && ctx.loopVariables && ctx.loopVariables.has(simpleNameStr)) {
      ...
      if (typeof loopVal === "number") return AstBuilder.int(loopVal);
      return AstBuilder.int(0);
    }
    ...
    if (node.parts.length > 1 && ctx.loopVariables) {
      ...
      if (firstPart && ctx.loopVariables.has(firstPart)) {
        ...
        if (loopVal instanceof ModelicaNameExpression) {
          ...
          const variable = ctx.dae.arenaGetVarByName(flatName);
    ...
    if (!isBuiltinVar && !node.global && ctx.classInstance && typeof ctx.classInstance.resolveName === "function") {
      ...
      let hasRedeclaredIntermediate = false;
      if (firstPartResolved !== null && resolved === null) {
        if (firstPartResolved instanceof ModelicaComponentInstance && node.parts.length >= 2) {
          const lastPart = node.parts[node.parts.length - 1]?.identifier?.text;
          if (lastPart && ["start", "fixed", "min", "max", "nominal", "stateSelect"].includes(lastPart)) {
            isAttribute = true;
          }
        }
        if (ctx.redeclareContext && ctx.redeclareContext.size > 0) {
          for (const part of node.parts) {
            const partName = part?.identifier?.text;
            if (partName && ctx.redeclareContext.has(partName)) {
              hasRedeclaredIntermediate = true;
              break;
            }
          }
        }
      ...
      if (
              ...
              if (
                ...
                if (localResolved === stackClass) {
                  ...
                  while (queue.length > 0) {
                    ...
                    if ("shortClassTarget" in cls && (cls as any).shortClassTarget) {
                      queue.push((cls as any).shortClassTarget);
                    }
                  }
                }
              }
        ...
        if (firstPartResolved !== null && resolved === null) {
          let existsInDAE = false;
          if (ctx.dae) {
            for (const v of ctx.dae.arenaVariables()) {
              if (v.name === name) {
                existsInDAE = true;
                break;
              }
        ...
        // It's an attribute access that wasn't resolved by resolveName.
        // We fallback to resolving the fully qualified name.
        name = this.#resolveFullyQualifiedName(rawName, ctx);
      }
        ...
        if (resolved instanceof ModelicaClassInstance) {
          ...
          if (classInst instanceof ModelicaEnumerationClassInstance) {
            ...
            for (const enumerationLiteral of classInst.enumerationLiterals ?? []) {
              if (enumerationLiteral.stringValue === memberName) {
                return new ModelicaEnumerationLiteral(
                  enumerationLiteral.ordinalValue,
                  enumerationLiteral.stringValue,
                  null,
                  classInst.name,
                );
              }
    ...
    if (ctx.classInstance instanceof ModelicaEnumerationClassInstance) {
      ...
      if (subscriptNodes.length > 0) {
          ...
          if (sub.flexible) {
            ...
            if (subExpr instanceof ModelicaRangeExpression) {
              ...
              if (startVal != null && stopVal != null && stepVal != null && stepVal !== 0) {
                const elements: ModelicaExpression[] = [];
                if (stepVal > 0) {
                  for (let v = startVal; v <= stopVal; v += stepVal) elements.push(AstBuilder.int(v));
                } else {
                  for (let v = startVal; v >= stopVal; v += stepVal) elements.push(AstBuilder.int(v));
                }
                subExpr = AstBuilder.array([elements.length], elements);
              }
            }

            subscripts.push(subExpr ?? AstBuilder.name("?"));
          }
        }
        ...
        if (resolvedFromFlattener.length === subscriptNodes.length) {
          ...
          const variable = ctx.dae.arenaGetVarByName(indexedName);
          if (variable) return variable;
        }
        ...
        const arraySize = ctx.dae.arenaGetArrayElements(arrayPrefix.slice(0, -1)).length;
        const interp = new ModelicaInterpreter(true);
        ...
        if (!hasLoopVarSubscript) {
          for (const sub of subscriptNodes) {
            ...
            const indexExpr = sub.expression.accept(interp, ctx.classInstance as any);
        ...
        if (hasSymbolicLoopVar) {
          return AstBuilder.subscript(AstBuilder.name(name), subscripts);
        } else {
      ...
      const variable = ctx.dae.arenaGetVarByName(name);
      if (variable) return variable;
      ...
      const prefix = name + "[";
      const arrayElements = ctx.dae.arenaGetArrayElements(prefix.slice(0, -1));
      ...
      if (arrayElements.length > 0) {
        ...
        if (firstSub.includes(",")) {
          ...
          const rowArrays = [...rows.values()].map((elems) => AstBuilder.array([elems.length], elems));
          const colCount = rowArrays.length > 0 ? (rowArrays[0]?.elements.length ?? 0) : 0;
          return AstBuilder.array([rowArrays.length, colCount], rowArrays);
        }
        return AstBuilder.array([arrayElements.length], arrayElements);
      }
      ...
      if (ctx.classInstance && ctx.classInstance.classKind !== ModelicaClassKind.FUNCTION) {
        ...
        if (firstName) {
          ...
          if (
            ...
            const hasTrueZeroDim = shape.some((d: number, i: number) => d === 0 && !subscripts[i]?.flexible);
            if (hasTrueZeroDim) {
              return AstBuilder.array([0], []);
            }
          }
      ...
      if (ctx.dae.arenaGetEncoded(name)) {
        return AstBuilder.name(name);
      }
    }
  ...
  private flattenEquations(
    ...
    const oldEquations = ctx.dae.equations;
    ctx.dae.equations = collected;
    try {
      for (const eq of equations) {
        eq.accept(this, ctx);
      }
    } finally {
      ctx.dae.equations = oldEquations;
    }
    return collected;
  }
    ...
    if (!values) {
      ...
      if (origExpr) {
        ...
        if ("parts" in origExpr) {
          const namedElement = ctx.classInstance.resolveComponentReference(origExpr as any);
          ...
          if (startExpr && stopExpr && "parts" in startExpr && "parts" in stopExpr) {
            const startElement = ctx.classInstance.resolveComponentReference(startExpr as any);
            const stopElement = ctx.classInstance.resolveComponentReference(stopExpr as any);
        ...
        if (enumClass && literalsToIterate) {
          ...
          for (const literal of literalsToIterate) {
            ...
            loopVars.set(indexName, AstBuilder.name(qualifiedName));
  ...
  #flattenForRange(
    ...
    const origExpr = fi.expression;
    const expr = origExpr?.accept(this, ctx) as ModelicaExpression;
    if (!expr) return null;

    // Expand enumeration TYPE references (e.g. `for e in E`)
    if (expr instanceof ModelicaNameExpression && origExpr && "parts" in origExpr) {
      const namedElement = ctx.classInstance.resolveComponentReference(origExpr as any);
      let enumClass: ModelicaEnumerationClassInstance | null = null;
      if (namedElement instanceof ModelicaEnumerationClassInstance) enumClass = namedElement;
      else if (namedElement instanceof ModelicaComponentInstance) {
        if (!namedElement.instantiated && !namedElement.instantiating) namedElement.instantiate();
        if (namedElement.classInstance instanceof ModelicaEnumerationClassInstance)
          enumClass = namedElement.classInstance;
    ...
    // Expand enumeration RANGE references (e.g. `for i in E.a : E.b`)
    if (expr instanceof ModelicaRangeExpression && origExpr && "startExpression" in (origExpr as any)) {
      const startNode = (origExpr as any).startExpression;
      const stopNode = (origExpr as any).stopExpression;
      if (startNode && stopNode && "parts" in startNode && "parts" in stopNode) {
        const startElement = ctx.classInstance.resolveComponentReference(startNode);
        const stopElement = ctx.classInstance.resolveComponentReference(stopNode);
        let enumClass: any = null;
        if (startElement?.isEnumerationLiteral) {
          const typeName = (startElement as any).enumerationTypeName;
          if (typeName) {
            ...
            enumClass = resolvedType instanceof ModelicaComponentInstance ? resolvedType.classInstance : resolvedType;
          }
        }

        if (enumClass && enumClass.enumerationLiterals && startElement && stopElement) {
          const startOrd = startElement.literalIndex;
          const stopOrd = stopElement.literalIndex;
          const elements: ModelicaExpression[] = [];
          for (const literal of enumClass.enumerationLiterals) {
            if (literal.ordinalValue >= startOrd && literal.ordinalValue <= stopOrd) {
              const typeName = enumClass.compositeName ?? "";
              elements.push(AstBuilder.name(typeName + "." + literal.stringValue));
            }
          }
          return AstBuilder.array([elements.length], elements);
        }
    ...
    if (expr instanceof ModelicaArray && expr.elements.length > 0) {
      ...
      if (first instanceof ModelicaIntegerLiteral && last instanceof ModelicaIntegerLiteral) {
        ...
        if (step !== null && step !== 0) {
          return new ModelicaRangeExpression(first, last, AstBuilder.int(step));
        }
      }
    }
  ...
  visitIfEquation(node: ModelicaIfEquationSyntaxNode, ctx: FlattenerContext): null {
    ...
    if (condition instanceof ModelicaBooleanLiteral) {
      ...
      const interp = new ModelicaInterpreter(true);
      const interpResult = node.condition.accept(interp, ctx.classInstance as any);
    ...
    if (target && source) {
      ...
      if (
        ...
        if (bracketIdx > 0) {
          ...
          effectiveTarget = AstBuilder.name(arrayBaseName);
        }
      }
      ...
      if (source) {
        ...
        if (src instanceof ModelicaFunctionCallExpression && !BUILTIN_FUNCTIONS.has(src.functionName)) {
          ...
          if (fnDef) {
            const outputCount = fnDef.legacyVariables.filter((v) => v.causality === "output").length;
            if (outputCount > 1) {
              source = AstBuilder.subscript(src, [AstBuilder.int(1)]);
            }
          }
        }
      }
  ...
  visitComplexAssignmentStatement(node: ModelicaComplexAssignmentStatementSyntaxNode, ctx: FlattenerContext): null {
    ...
    if (nonNullTargets.length === 1 && nonNullTargets[0]) {
      ...
      const subscripted = AstBuilder.subscript(source, [AstBuilder.int(index + 1)]);
      if (target)
        ctx.stmtCollector.push(
  ...
  visitForStatement(node: ModelicaForStatementSyntaxNode, ctx: FlattenerContext): null {
    ...
    for (const forIndex of node.forIndexes) {
      const indexName = forIndex?.identifier?.text;
      if (indexName) loopVars.set(indexName, AstBuilder.name(indexName));
    }
    ...
    for (let i = node.forIndexes.length - 1; i >= 0; i--) {
      ...
      if (range instanceof ModelicaNameExpression && forIndex.expression && "parts" in forIndex.expression) {
        const namedElement = ctx.classInstance.resolveComponentReference(forIndex.expression as any);
        let enumClass: ModelicaEnumerationClassInstance | null = null;
        if (namedElement instanceof ModelicaEnumerationClassInstance) {
          enumClass = namedElement;
        } else if (namedElement instanceof ModelicaComponentInstance) {
        ...
        if (enumClass?.enumerationLiterals && enumClass.enumerationLiterals.length > 0) {
          ...
          range = AstBuilder.array([elements.length], elements);
        }
      }
  ...
  #findDimensionForIndex(
    ...
    for (const part of ref.parts) {
      ...
      for (let dimIdx = 0; dimIdx < subscripts.length; dimIdx++) {
        ...
        if (dimSize > 0) {
          return new ModelicaRangeExpression(AstBuilder.int(1), AstBuilder.int(dimSize), null);
        }
      }
    }
    return null;
  ...
  #getArrayDimensionSize(qualifiedName: string, dimIdx: number, totalDims: number, ctx: FlattenerContext): number {
    ...
    const arrayVars = ctx.dae.arenaGetArrayElements(prefix.slice(0, -1));
  ...
  visitRangeExpression(node: ModelicaRangeExpressionSyntaxNode, ctx: FlattenerContext): ModelicaExpression | null {
    ...
    if (start && typeof (start as any).accept !== "function") {
      console.error("visitRangeExpression START IS POJO:", start, start.constructor.name);
    }
    ...
    if (startVal !== null && stopVal !== null && stepVal !== null) {
      ...
      if (stepVal > 0) {
        for (let v = startVal; v <= stopVal; v += stepVal) {
          ...
          elements.push(AstBuilder.int(v));
        }
      } else {
        for (let v = startVal; v >= stopVal; v += stepVal) {
          if (elements.length >= 10000) break;
      ...
      if (stp === 0 || startR + stp === startR) {
        ...
        if (stp > 0) {
          for (let v = startR; v <= stopR + 1e-10; v += stp) {
            if (elements.length >= 10000) break;
            elements.push(AstBuilder.real(v));
          }
        } else {
          for (let v = startR; v >= stopR - 1e-10; v += stp) {
            if (elements.length >= 10000) break;
  ...
  /**
   * Expand `connect(a: any, b: any)` into scalar equations:
    ...
    for (const [localName, info1] of leaves1) {
      ...
      if (info1.isFlow) {
        ...
        ctx.dae.equations.push(
          new ModelicaSimpleEquation(AstBuilder.name(info1.fullName), AstBuilder.name(info2.fullName)),
        );
      }
    }
  ...
  #resolveConnectName(ref: ModelicaComponentReferenceSyntaxNode, ctx: FlattenerContext): string | null {
    const parts = ref.parts.map((p) => {
      ...
      if (p.arraySubscripts?.subscripts?.length) {
        const subs: string[] = [];
        for (const sub of p.arraySubscripts.subscripts) {
          const val = sub.expression?.accept(new ModelicaInterpreter(true), ctx.classInstance as any);
  ...
  private collapseArrayToName(expr: ModelicaExpression): ModelicaExpression {
    ...
    if (allVars.every((v) => v.name.startsWith(baseName + "["))) {
      return AstBuilder.name(baseName);
    }
    return expr;
  }
  ...
  private collapseArraysInExpr(expr: ModelicaExpression): ModelicaExpression {
    ...
    if (expr instanceof ModelicaBinaryExpression) {
      ...
      if (left !== expr.operand1 || right !== expr.operand2) {
        return AstBuilder.binary(expr.operator, left, right);
      }
    }
    return expr;
  }
  ...
  visitSimpleEquation(node: ModelicaSimpleEquationSyntaxNode, ctx: FlattenerContext): null {
    ...
    const hasEmptyArray = (e: ModelicaExpression | null | undefined): boolean =>
      e instanceof ModelicaArray && e.shape.some((d: any) => d === 0);
    ...
    if (
      ...
      if (lhsName) {
        ...
        const hasIndexedVars = ctx.dae.arenaHasArrayElements(rootName);
        ...
        if (isScalarLHS) {
          ...
          const arrayVars = [...ctx.dae.arenaVariables()]
          ...
          if (arrayVars.length === flatRhs.length) {
            for (let i = 0; i < arrayVars.length; i++) {
              ...
              const lhs = AstBuilder.name(arrayVar.name);
              let rhs = flatRhs[i];
              if (!rhs) continue;
    ...
    if (expression1 && expression2) {
      ...
      if (expression2 instanceof ModelicaFunctionCallExpression) {
        ...
        if (funcDef) {
          ...
          const outputs = funcDef.legacyVariables.filter((v) => v.causality === "output");
          if (outputs.length > 1) {
            if (!(expression1 instanceof ModelicaTupleExpression)) {
              ...
              // Pad existing tuple: (a: any, b: any) = func() -> (a, b, _) = func()
      ...
      if (expression1 instanceof ModelicaTupleExpression && expression2 instanceof ModelicaArray) {
        ...
        for (const lhsElem of expression1.elements) {
          if (lhsElem instanceof ModelicaArray) {
            ...
            rhsElements.push(AstBuilder.array([subValues.length], subValues));
          } else {
      ...
      if (expression1 instanceof ModelicaArray && expression2 instanceof ModelicaFunctionCallExpression) {
        ...
        const hasScalarInputsOnly = funcDef
          ? funcDef.legacyVariables.filter((v) => v.causality === "input").every((v) => !v.name.includes("["))
          : false;
        ...
        if (hasScalarInputsOnly && hasArrayArgs && lhsElements.length > 0) {
            ...
            if (arg instanceof ModelicaBinaryExpression) {
                    ...
                    if (fn === "fill" || fn === "ones" || fn === "zeros") {
                      ...
                      const resolveExpr = (e: ModelicaExpression): ModelicaExpression => {
                        ...
                        if (e instanceof ModelicaNameExpression) {
                          const v = ctx.dae.arenaGetVarByName(e.name);
                        ...
                        if (e instanceof ModelicaUnaryExpression) {
                          ...
                          if (
                            ...
                            return AstBuilder.int(-op.value);
                          }
                          return AstBuilder.unary(e.operator, op);
                        }
                      ...
                      if (fn === "fill" && resolvedArgs.length >= 2) {
                        ...
                        if (fillValue instanceof ModelicaRealLiteral && Number.isInteger(fillValue.value)) {
                          fillValue = AstBuilder.int(fillValue.value);
                        }
                      ...
                      if (fn === "ones" && resolvedArgs.length >= 1) {
                        ...
                        if (dim instanceof ModelicaIntegerLiteral) {
                          const arr = buildFilledArray([dim.value], AstBuilder.int(1));
                          return [...arr.flatElements];
                        }
                      }
                    }
              ...
              if (op1Arr && op2Arr && op1Arr.length === count && op2Arr.length === count) {
                ...
                for (let k = 0; k < count; k++) {
                  ...
                  if (r) results.push(r);
                  else results.push(AstBuilder.binary(arg.operator, e1, e2));
                }
                return results;
              }
              // One operand is an array, the other is scalar — distribute
              if (op1Arr && op1Arr.length === count) {
                return op1Arr.map((el) => AstBuilder.binary(arg.operator, el, arg.operand2));
              }
              if (op2Arr && op2Arr.length === count) {
                return op2Arr.map((el) => AstBuilder.binary(arg.operator, arg.operand1, el));
              }
            }
          ...
          for (let i = 0; i < count; i++) {
            ...
            const scalarCall = AstBuilder.call(rhsCall.functionName, coercedArgs);
        ...
        if (elements.length > 0) {
          ...
          if (firstRoot && rootNames.every((r) => r === firstRoot)) {
            expression1 = AstBuilder.name(firstRoot);
          }
        }
      }
        ...
        if (expr instanceof ModelicaBinaryExpression) {
          ...
          if (op1Arr || op2Arr) {
            ...
            for (let i = 0; i < count; i++) {
              ...
              elements.push(AstBuilder.binary(expr.operator, e1 as ModelicaExpression, e2 as ModelicaExpression));
            }
            return AstBuilder.array([count], elements);
          }
        }
        if (expr instanceof ModelicaUnaryExpression) {
          const opArr = tryExpandToLength(expr.operand, count);
          if (opArr) {
            const elements = opArr.elements.map((e) => AstBuilder.unary(expr.operator, e));
            return AstBuilder.array([count], elements);
          }
        }
        return null;
      ...
      const isRecordConstructorCall = (e: ModelicaExpression): boolean => {
        ...
        if (!fd) return false;
        const outVar = fd.legacyVariables.find((v) => v.causality === "output");
        return !!(outVar?.customTypeName && e.functionName === outVar.customTypeName);
      };
      const isArrayName = (e: ModelicaExpression): boolean =>
        e instanceof ModelicaNameExpression && ctx.dae.arenaHasArrayElements(e.name);
      ...
      const isArrayTarget = (expr: ModelicaExpression): boolean => {
        if (expr instanceof ModelicaNameExpression) {
          const foundVar = ctx.dae.arenaGetVarByName(expr.name);
  ...
  visitStringLiteral(node: ModelicaStringLiteralSyntaxNode): ModelicaExpression | null {
    return AstBuilder.string(node.text ?? "");
  }
  ...
  visitUnaryExpression(node: ModelicaUnaryExpressionSyntaxNode, ctx: FlattenerContext): ModelicaExpression | null {
    ...
    if (operator === ModelicaUnaryOperator.UNARY_MINUS) {
      if (operand instanceof ModelicaRealLiteral) return AstBuilder.real(-operand.value);
      if (operand instanceof ModelicaIntegerLiteral) return AstBuilder.int(-operand.value);
      ...
      if (
        ...
        const negatedFirst = AstBuilder.unary(ModelicaUnaryOperator.UNARY_MINUS, operand.operand1);
        return AstBuilder.binary(operand.operator, negatedFirst, operand.operand2);
      }
      ...
      if (operatorInfo) {
        const { qualifiedName, resolvedClass } = operatorInfo;
        this.#collectFunctionDefinition(qualifiedName, ctx, resolvedClass);
        return AstBuilder.call(qualifiedName, [operand]);
      }
    }
    ...
    return AstBuilder.unary(operator, operand);
  }

  visitUnsignedIntegerLiteral(node: ModelicaUnsignedIntegerLiteralSyntaxNode): ModelicaIntegerLiteral | null {
    return AstBuilder.int(node.value);
  }

  visitUnsignedRealLiteral(node: ModelicaUnsignedRealLiteralSyntaxNode): ModelicaRealLiteral | null {
    return AstBuilder.real(node.value, node.text ?? undefined);
  }
}

/**
 * Standalone helper to evaluate a symbolic expression to an array of integer values.
 * Used in #foldExpression for comprehension expansion (where private class methods
 * from ModelicaSyntaxFlattener are not accessible).
 */
function evaluateRangeExpr(expr: ModelicaExpression): number[] | null {
  if (expr instanceof ModelicaRangeExpression) {
    const startVal = expr.start instanceof ModelicaIntegerLiteral ? expr.start.value : null;
    const endVal = expr.end instanceof ModelicaIntegerLiteral ? expr.end.value : null;
    if (startVal === null || endVal === null) return null;
    const stepVal = expr.step ? (expr.step instanceof ModelicaIntegerLiteral ? expr.step.value : null) : 1;
    if (stepVal === null || stepVal === 0) return null;
    const result: number[] = [];
    if (stepVal > 0) {
      for (let i = startVal; i <= endVal; i += stepVal) result.push(i);
  ...
  if (expr instanceof ModelicaArray) {
    const result: number[] = [];
    for (const el of expr.elements) {
      if (el instanceof ModelicaIntegerLiteral) result.push(el.value);
      else if (el instanceof ModelicaRealLiteral) result.push(Math.round(el.value));
      else return null;
    }
    return result;
  }
...
function substituteIterator(expr: ModelicaExpression, iterName: string, value: ModelicaExpression): ModelicaExpression {
  ...
  // Handle dotted names like `i.x` where `i` is the iterator → `r[1].x`
  if (expr instanceof ModelicaNameExpression && expr.name.startsWith(iterName + ".")) {
    const suffix = expr.name.substring(iterName.length); // ".x"
    if (value instanceof ModelicaNameExpression) {
      return AstBuilder.name(value.name + suffix);
    }
  }
  if (expr instanceof ModelicaBinaryExpression) {
    const op1 = substituteIterator(expr.operand1, iterName, value);
    const op2 = substituteIterator(expr.operand2, iterName, value);
    return op1 === expr.operand1 && op2 === expr.operand2 ? expr : AstBuilder.binary(expr.operator, op1, op2);
  }
  if (expr instanceof ModelicaUnaryExpression) {
    const op = substituteIterator(expr.operand, iterName, value);
    return op === expr.operand ? expr : AstBuilder.unary(expr.operator, op);
  }
  if (expr instanceof ModelicaFunctionCallExpression) {
  ...
  if (expr instanceof ModelicaSubscriptedExpression) {
    ...
    return AstBuilder.subscript(base, subs);
  }
  if (expr instanceof ModelicaArray) {
    const els = expr.elements.map((e) => substituteIterator(e, iterName, value));
    return AstBuilder.array(expr.shape, els);
  }
...
function castToReal(expression: ModelicaExpression | null): ModelicaExpression | null {
  ...
  // Already wrapped in /*Real*/ — don't double-wrap
  if (expression instanceof ModelicaFunctionCallExpression && expression.functionName === "/*Real*/") return expression;
  if (expression instanceof ModelicaIntegerLiteral) return AstBuilder.real(expression.value);
  ...
  if (expression instanceof ModelicaUnaryExpression) {
    ...
    if (operand !== expression.operand) return AstBuilder.unary(expression.operator, operand);
  }
  if (expression instanceof ModelicaBinaryExpression) {
    ...
    if (op1 !== expression.operand1 || op2 !== expression.operand2)
      return AstBuilder.binary(expression.operator, op1, op2);
  }
  if (expression instanceof ModelicaFunctionCallExpression) {
    ...
    if (args.some((a, i) => a !== expression.args[i])) return AstBuilder.call(expression.functionName, args);
  }
...
function expressionHasNameRefs(expr: ModelicaExpression, dae: ModelicaDAE): boolean {
  if (expr instanceof ModelicaNameExpression) {
    ...
    const v = dae.arenaGetVarByName(expr.name);
...
function coerceToReal(expression: ModelicaExpression | null, dae?: ModelicaDAE): ModelicaExpression | null {
  ...
  if (expression instanceof ModelicaIntegerVariable) {
    return AstBuilder.call("/*Real*/", [expression]);
  }
  ...
  if (expression instanceof ModelicaNameExpression && dae) {
    ...
    if (builtinType === "Real") {
      ...
      const variable = dae.arenaGetVarByName(expression.name);
      if (variable instanceof ModelicaRealVariable) {
        // Already Real-typed, no coercion needed
      } else if (!variable) {
        // Check for array name references — e.g., "B" when "B[1,1]", "B[1,2]" etc. exist
        const arrayEls = dae.arenaGetArrayElements(expression.name);
        ...
        if (arrayEl instanceof ModelicaRealVariable) {
          ...
          const encodedMatch = dae.arenaGetEncoded(baseName);
...
function isRealTyped(expr: ModelicaExpression, dae?: ModelicaDAE): boolean {
  ...
  if (expr instanceof ModelicaNameExpression && dae) {
    const exactMatch = dae.arenaGetVarByName(expr.name);
    if (exactMatch instanceof ModelicaRealVariable && !exactMatch.customTypeName) return true;

    const arrayEls = dae.arenaGetArrayElements(expr.name);
    ...
    const encodedMatch = dae.arenaGetEncoded(expr.name);
...
function isIntegerTyped(expr: ModelicaExpression, dae?: ModelicaDAE): boolean {
  ...
  if (expr instanceof ModelicaNameExpression && dae) {
    ...
    const intArrayEls = dae.arenaGetArrayElements(expr.name);
...
function expandArrayVariable(variable: ModelicaVariable): ModelicaArray | null {
  ...
  for (let i = 1; i <= size; i++) {
    elements.push(AstBuilder.name(`${baseName}[${i}]`));
  }
  return AstBuilder.array([size], elements);
}
...
function canonicalizeBinaryExpression(
  ...
  if (
    ...
    return AstBuilder.string(operand1.value + operand2.value);
  }
  ...
  if (dae && operand1 instanceof ModelicaNameExpression) {
    ...
    const encoded = dae.arenaGetEncoded(op1Name);
    if (encoded) {
  ...
  if (dae && operand2 instanceof ModelicaNameExpression) {
    ...
    const encoded = dae.arenaGetEncoded(op2Name);
    if (encoded) {
      const expanded = expandArrayVariable(encoded);
  ...
  if (operand1 instanceof ModelicaArray && operand2 instanceof ModelicaArray) {
    ...
    if (
      ...
      if (nCols === vecLen && nCols > 0 && nRows > 0) {
        ...
        for (let i = 0; i < nRows; i++) {
          ...
          resultElements.push(dotProduct ?? AstBuilder.int(0));
        }
        return AstBuilder.array([nRows], resultElements);
      }
    }
    ...
    if (
      ...
      if (n === m && n > 0) {
        ...
        return dotProduct ?? AstBuilder.int(0);
      }
    }
    ...
    if (
      ...
      if (vecLen === nRows && vecLen > 0 && nCols > 0) {
        ...
        return AstBuilder.array([nCols], resultElements);
      }
    }
      ...
      if (mCols === nRows && mCols > 0 && mRows > 0 && nCols > 0) {
        ...
        for (let i = 0; i < mRows; i++) {
          ...
          for (let j = 0; j < nCols; j++) {
            ...
            resultRow.push(dotProduct ?? AstBuilder.int(0));
          }
          rowArrays.push(AstBuilder.array([nCols], resultRow));
        }
        return AstBuilder.array([mRows, nCols], rowArrays);
      }
    }
    if (scalarOp === "+" || scalarOp === "-" || scalarOp === "*" || scalarOp === "/" || scalarOp === "^") {
      if (operand1.elements.length === operand2.elements.length) {
        ...
        return AstBuilder.array(operand1.shape, newElements);
      }
    }
    ...
    if (scalarOp === "+" || scalarOp === "-" || scalarOp === "*" || scalarOp === "/" || scalarOp === "^") {
      ...
      if ((scalarOp === "+" || scalarOp === "-") && !isElementwiseOp) {
        ...
        return AstBuilder.binary(operator, operand1, operand2);
      }
      ...
      const newElements = operand1.elements.map((e) => AstBuilder.binary(scalarOp, e, operand2));
      return AstBuilder.array(operand1.shape, newElements);
    }
  } else if (!(operand1 instanceof ModelicaArray) && operand2 instanceof ModelicaArray) {
    ...
    if (scalarOp === "+" || scalarOp === "-" || scalarOp === "*" || scalarOp === "/" || scalarOp === "^") {
      ...
      if (isElementwiseOp) {
        ...
        return AstBuilder.array(operand2.shape, newElements);
      }
      // Non-element-wise: preserve source operand order (scalar * array)
      const newElements = (operand2 as ModelicaArray).elements.map((e) => AstBuilder.binary(scalarOp, operand1, e));
      return AstBuilder.array(operand2.shape, newElements);
    }
  }
  ...
  if (
    ...
    if (result != null && Number.isFinite(result)) {
      ...
      if (
        ...
        return AstBuilder.int(result);
      }
      return AstBuilder.real(result);
    }
    ...
    if (boolResult != null) return AstBuilder.bool(boolResult);
  }
  ...
  const isEmptyArray = (e: ModelicaExpression) => e instanceof ModelicaArray && e.shape.some((d: any) => d === 0);
  ...
  if (operator === ModelicaBinaryOperator.DIVISION && operand2 instanceof ModelicaIntegerLiteral) {
    ...
    const castOp2 = AstBuilder.real(operand2.value);
    return AstBuilder.binary(ModelicaBinaryOperator.DIVISION, castOp1, castOp2);
  }
    ...
    const tryFold = (baseExpr: ModelicaExpression, multExpr: ModelicaExpression): ModelicaExpression | null => {
      if (multExpr instanceof ModelicaBinaryExpression && multExpr.operator === ModelicaBinaryOperator.MULTIPLICATION) {
        ...
        if (isLiteral(multExpr.operand1) && multExpr.operand2.hash === baseExpr.hash) {
          ...
          const one = c instanceof ModelicaIntegerLiteral ? AstBuilder.int(1) : AstBuilder.real(1.0);
  ...
  if (dae) {
    if (isRealTyped(operand1, dae)) {
      ...
      if (op2 !== operand2) {
        return AstBuilder.binary(operator, operand1, op2);
      }
    }
    if (isRealTyped(operand2, dae)) {
      const op1 = wrapIntegerAsReal(operand1, dae);
      if (op1 !== operand1) {
        return AstBuilder.binary(operator, op1, operand2);
      }
    }
  }
...
function wrapIntegerAsReal(expr: ModelicaExpression, dae?: ModelicaDAE): ModelicaExpression {
  if (expr instanceof ModelicaIntegerVariable) {
    return AstBuilder.call("/*Real*/", [expr]);
  }
  ...
  [
    ...
    [
      ...
      (args) => {
        if (args.length < 1) return null;
        const arg0 = args[0];
        if (arg0 instanceof ModelicaIntegerLiteral) return AstBuilder.string(String(arg0.value));
        if (arg0 instanceof ModelicaRealLiteral) return AstBuilder.string(String(arg0.value));
        if (arg0 instanceof ModelicaBooleanLiteral) return AstBuilder.string(arg0.value ? "true" : "false");
        if (arg0 instanceof ModelicaStringLiteral) return arg0;
        if (arg0 instanceof ModelicaEnumerationLiteral) return AstBuilder.string(arg0.stringValue);
        return null;
      },
    ],
    ...
    [
      ...
      (args) => {
        if (args.length === 1 && args[0] instanceof ModelicaEnumerationLiteral) {
          return AstBuilder.int(args[0].ordinalValue);
        }
        return null;
      },
    ],
    ...
    [
      ...
      (args) => {
        ...
        if (args[0] instanceof ModelicaBooleanLiteral && args[1] instanceof ModelicaBooleanLiteral) {
          return AstBuilder.bool(args[0].value && args[1].value);
        }
...
function tryFoldBuiltinFunction(functionName: string, args: ModelicaExpression[]): ModelicaExpression | null {
  ...
  if (args.length === 0 && def.identityValue !== undefined) {
    return Number.isInteger(def.identityValue) ? AstBuilder.int(def.identityValue) : AstBuilder.real(def.identityValue);
  }
  ...
  if (numArgs.length === 1 && def.fold1) {
    ...
    if (def.outputType === "Integer") return AstBuilder.int(result);
    return AstBuilder.real(result);
  }
