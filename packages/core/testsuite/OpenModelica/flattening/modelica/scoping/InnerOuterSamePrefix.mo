      ...
      package Visualizers  "3-dimensional visual objects used for animation"
        ...
        package Internal  "Visualizers that will be replaced by improved versions in the future (do not use them)"
          ...
          model Lines  "Visualizing a set of lines as cylinders with variable size, e.g., used to display characters (no Frame connector)"
            input Modelica.Mechanics.MultiBody.Frames.Orientation R = .Modelica.Mechanics.MultiBody.Frames.nullRotation() "Orientation object to rotate the world frame into the object frame" 
            input .Modelica.SIunits.Position[3] r = {0, 0, 0} "Position vector from origin of world frame to origin of object frame, resolved in world frame" 
            input .Modelica.SIunits.Position[3] r_lines = {0, 0, 0} "Position vector from origin of object frame to the origin of 'lines' frame, resolved in object frame" 
            ...
            input .Modelica.SIunits.Position[:, 2, 2] lines = zeros(0, 2, 2) "List of start and end points of cylinders resolved in an x-y frame defined by n_x, n_y, e.g., {[0,0;1,1], [0,1;1,0], [2,0; 3,1]}" 
            input .Modelica.SIunits.Length diameter(min = 0) = 0.05 "Diameter of the cylinders defined by lines" 
      ...
      partial package PartialModelicaServices  "Interfaces of components requiring a tool specific implementation"
        ...
        package Animation  "Models and functions for 3-dim. animation"
          ...
          partial model PartialShape  "Interface for 3D animation of elementary shapes"
            ...
            input .Modelica.Mechanics.MultiBody.Frames.Orientation R = .Modelica.Mechanics.MultiBody.Frames.nullRotation() "Orientation object to rotate the world frame into the object frame" 
            input .Modelica.SIunits.Position[3] r = {0, 0, 0} "Position vector from origin of world frame to origin of object frame, resolved in world frame" 
            input .Modelica.SIunits.Position[3] r_shape = {0, 0, 0} "Position vector from origin of object frame to shape origin, resolved in object frame" 
            ...
            input .Modelica.SIunits.Length length = 0 "Length of visual object" 
            input .Modelica.SIunits.Length width = 0 "Width of visual object" 
            input .Modelica.SIunits.Length height = 0 "Height of visual object" 
            input .Modelica.Mechanics.MultiBody.Types.ShapeExtra extra = 0.0 "Additional size data for some of the shape types" 
...
// Result:
// Error processing file: InnerOuterSamePrefix.mo
...
// Error: Error occurred while flattening model InnerOuterSamePrefix.mo [BUG: #2650]
//
