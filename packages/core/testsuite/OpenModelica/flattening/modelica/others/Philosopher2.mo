...
//     (u1, s2) := Philosopher.Random.random({s1[1], s1[2], s1[3]});
//     (u2, s1) := Philosopher.Random.random({s2[1], s2[2], s2[3]});
//     z := 1.7155277699214135 * (-0.5 + u1) / u2;
//     zz := 0.25 * z ^ 2.0;
//     my_break := zz <= (-log(u2));
//   end while;
//   x := mu + z * sigma;
...
//     so[3] := 1.0;
//   end if;
//   x := 3.3037100664045725e-5 * so[1] + 3.2995677566238825e-5 * so[2] + 3.307972213033411e-4 * so[3] - div(3.3037100664045725e-5 * so[1] + 3.2995677566238825e-5 * so[2] + 3.307972213033411e-4 * so[3], 1.0);
// end Philosopher.Random.random;
//
