...
export interface FmuArchiveOptions extends FmuOptions {
  /** Target export format: "wasm" | "c" | "js" | "all" (default: "all"). */
  ...
  /** Include JavaScript model in resources/ (default: true). */
  includeJs?: boolean;
...
export function buildFmuArchive(
  ...
  const shouldIncludeSources =
    target === "c" || target === "all" || (options.includeSources !== false && target !== "wasm" && target !== "js");
  const shouldIncludeWasm = target === "wasm" || target === "all" || options.includeWasm || !!options.wasmBinary;
  const shouldIncludeJs =
    target === "js" || target === "all" || (options.includeJs !== false && target !== "c" && target !== "wasm");
  ...
  // ── JavaScript & AssemblyScript models in resources ──
  if (shouldIncludeJs) {
  ...
  if (shouldIncludeWasm) {
    if (options.includeWasm) {
      ...
      const wasmBuildInstructions = [
        ...
        `# Requires Emscripten (https://emscripten.org/) or AssemblyScript (asc)`,
        ``,
    ...
    if (options.wasmBinary) {
      ...
      files.set(`binaries/wasm32-unknown-unknown/${id}.wasm`, options.wasmBinary);
    }
    if (options.wasmJsGlue) {
      files.set(`binaries/wasm32/${id}.js`, encoder.encode(options.wasmJsGlue));
      files.set(`binaries/wasm32-unknown-unknown/${id}.js`, encoder.encode(options.wasmJsGlue));
    }
  }
