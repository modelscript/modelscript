...
import { type DAEBuilder, BinOp, EqKind, ExprKind, UnaryOp } from "@modelscript/runtime";
 ...
 */
export function generateFmuAsSources(dae: DAEBuilder, fmuResult: FmuResult, options: FmuOptions): string {
...
function exprToAs(dae: DAEBuilder, id: number): string {
  if (id < 0) return "0.0";
...
function conditionToZeroCrossingAs(dae: DAEBuilder, id: number): string {
  if (id < 0) return "0.0";
...
function generateModelAs(id: string, nVars: number, nStates: number, dae: DAEBuilder, result: FmuResult): string {
  const lines: string[] = [];
