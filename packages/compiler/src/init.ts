 ...
 *
 * Creates:
 *   package.json     — deps (tree-sitter-cli, web-tree-sitter, @modelscript/compiler) + scripts
...
export function initProject(options: InitOptions): void {
  ...
  const pkg = {
    ...
    description: existingPkg.description ?? `Tree-sitter grammar for ${langName} (via @modelscript/compiler)`,
    ...
    dependencies: {
      ...
      "@modelscript/compiler": existingPkg.dependencies?.["@modelscript/compiler"] ?? "*",
