 ...
 * `field(name, repeat(...))` or `field(name, repeat1(...))`, we look at the
...
export function extractRecoverySpecs(langConfig: any, $: Record<string, any>): RecoverySpec[] {
  ...
  for (const [ruleName, ruleFn] of Object.entries<any>(langConfig.rules)) {
    ...
    for (let i = 0; i < args.length; i++) {
      ...
      // Look for field(name, repeat(...)) or field(name, repeat1(...))
      if (arg?.type !== "field") continue;
      const fieldContent = arg.arg;
      if (!fieldContent) continue;
      if (fieldContent.type !== "repeat" && fieldContent.type !== "repeat1") continue;
 ...
 * Handles: string, seq(first...), optional(inner), field(name, inner), sym
 */
