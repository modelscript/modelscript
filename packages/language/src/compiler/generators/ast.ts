...
interface FieldInfo {
  name: string;
  /** Whether the field is wrapped in repeat() / repeat1() (i.e. array). */
  isList: boolean;
 ...
 * Walks through seq, choice, optional, repeat, etc. to find all field() nodes.
 */

/**
 * Recursively infer the TypeScript return type (model class) from a grammar rule.
 *
 * This function walks down the syntax tree to figure out what type of AST node
 * will be generated by Tree-sitter for a given rule. It follows `sym` references
 * to resolve foreign rules, checks for union types (`choice`), arrays (`seq`),
 * and handles fallbacks to `SemanticNode` or `string` for primitive tokens.
 *
 * @param rule The rule AST node to infer the type for.
 * @param langConfig The evaluated language config object.
 * @param $ The proxy object used for rule evaluation.
 * @param visited A Set of visited rule names to prevent infinite recursion on recursive rules.
 * @returns The inferred TypeScript type as a string (e.g., "ExpressionNode", "string", "SemanticNode").
 */
...
/**
 * Recursively extracts all `field()` instances from a rule AST node.
 *
 * This function explores sequence (`seq`), choice (`choice`), optional (`opt`),
 * and repetition (`rep`/`rep1`) nodes to locate all `field("name", ...)`
 * annotations. It gathers typing and structural information to generate
 * the strongly typed AST wrapper classes.
 *
 * @param rule The rule AST node to extract fields from.
 * @param langConfig The evaluated language config object.
 * @param $ The proxy object used for rule evaluation.
 * @returns An array of FieldInfo objects describing the structure of each field.
 */
...
/**
 * Extracts class generation specifications from a language configuration.
 *
 * Scans all rules in the `langConfig`. If a rule is wrapped in `def()` and has
 * a `model` option defined, it extracts all fields, inheritance structures,
 * and metadata keys needed to generate the corresponding AST wrapper class.
 *
 * @param langConfig The evaluated language configuration.
 * @param $ The proxy object used for rule evaluation.
 * @returns An array of ClassSpec objects, each representing a future TypeScript class.
 */
 ...
 */
/**
 * Generates the TypeScript source code for the AST wrapper classes.
 *
 * This generator produces:
 * 1. Base class `SemanticNode` with generic tree-walking and querying capabilities.
 * 2. Specialized subclasses for every rule defined with `def({ model: ... })`.
 * 3. Typed property getters for all tree-sitter fields, correctly typing them
 *    as arrays, optionals, strings, or specific AST node types based on inference.
 * 4. Bound query getters to execute compiled tree-sitter queries.
 *
 * @param specs An array of ClassSpecs extracted from the language config.
 * @param langName The name of the language (used for prefixes/namespaces).
 * @returns The complete TypeScript source string for the AST classes.
 */
export function generateAstClasses(specs: ClassSpec[], langName: string): string {
  ...
  lines.push(``);
  lines.push(
    `import type { QueryDB, SymbolEntry, SymbolId, SpecializationArgs } from "@modelscript/language/compiler";`,
  );
  lines.push(`import { SemanticNode, GenericNode, type SemanticVisitor } from "@modelscript/language/compiler";`);
  lines.push(``);
...
function generateClass(spec: ClassSpec, visitorName: string): string[] {
  ...
  if (model.properties && Object.keys(model.properties).length > 0) {
    ...
    for (const [propName, propType] of Object.entries(model.properties)) {
      ...
      const isNullable = typeof propType === "string" && propType.includes("null");
