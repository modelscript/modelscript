...
async function generate(fileArg: string) {
  ...
  const combinedConfigContent =
    `import type { ${imports.join(", ")} } from "@modelscript/language/compiler";\n\n` +
    indexerContent +
    "\n" +
    refContent +
    "\n" +
...
async function playground(_fileArg: string) {
  console.log("Playground command has moved. Please run 'npx modelscript playground' from @modelscript/cli.");
}
