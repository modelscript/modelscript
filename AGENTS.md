...
9. **WASM Parser:** The project uses a custom, native WebAssembly GLR parser (`parser.wasm`) generated by `@modelscript/language`'s compiler (`buildParser`). Never import or reference `tree-sitter`. The parser is initialized via `createWasmParser`:
   ```typescript
...
| --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `languages/modelica/` | Modelica DSL definition, WASM GLR parser (`buildParser`), `ModelicaFlattener`, in-WASM flattener kernel (`flattener-wasm`)                                                                                                                           |
