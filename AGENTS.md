...
**IMPORTANT:** The compiler operates on a data-oriented struct-of-arrays architecture in WebAssembly linear memory (`DAEBuilder`, `ASTNode` pointers). Never instantiate heavy AST wrapper objects (`ModelicaExpressionSyntaxNode`) for expressions or flattening logic.
...
3. In the arena-native pipeline (`ModelicaFlattener` / `flattener-wasm`), CST nodes are accessed via `db.cstNode(classId)` and lowered directly into `DAEBuilder`.

## Compilation Pipeline Architecture

There is a single canonical flattening pipeline in the codebase: the arena-native pipeline (`Context.flattenArena()` → `ModelicaFlattener` / `flattener-wasm` → `DAEBuilder`).
...
| --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| `languages/modelica/` | Modelica DSL definition, WASM GLR parser, `ModelicaFlattener`, in-WASM flattener kernel (`flattener-wasm`)                                           |
...
| `packages/runtime/`   | Linear-memory WebAssembly DAE arena, `DAEBuilder`, `ArenaDAEPrinter`, Salsa `QueryEngine`, `WorkspaceIndex`, BLT solver, evaluator, two-tier storage |
